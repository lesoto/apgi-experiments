...
from .monitoring_dashboard import (
    LiveEEGMonitor, PupillometryMonitor, CardiacMonitor,
    RealTimeParameterEstimateUpdater, QualityAlertSystem
)
...
class RealTimePlotManager:
    ...
    def update_eeg_plot(self, timestamp: float, quality_score: float, artifact_rate: float):
    ...
    def update_parameter_plot(self, timestamp: float, param_name: str, mean: float, ci_lower: float, ci_upper: float):
        ...
        for t, name, m, l, u in self.param_data:
            if name not in param_groups:
                param_groups[name] = {'times': [], 'means': [], 'lower': [], 'upper': []}
...
class EnhancedMonitoringDashboard:
    ...
    def _initialize_components(self):
        ...
        self.params_monitor = RealTimeParameterEstimateUpdater(self.params_monitor_frame)
