...
logger = get_logger("test_execution_panel") if 'get_logger' in globals() else logging.getLogger("test_execution_panel")
...
class TestExecutionPanel(ctk.CTkFrame):
    ...
    def __init__(self, parent, test_name: str, controller: Optional[MainApplicationController] = None,
                 progress_callback: Optional[Callable] = None, log_callback: Optional[Callable] = None,
    ...
    def _create_description_section(self):
        ...
        desc_text.insert("1.0", descriptions.get(self.test_name, "Falsification test for APGI Framework"))
    ...
    def _create_parameters_section(self):
        ...
        self._create_parameter_row(params_frame, "n_trials", "Number of Trials", 1000, "Total number of trials to run")
        
        # Test-specific parameters
        if self.test_name in ["Soma-Bias", "Threshold Insensitivity", "Cross-Species Validation"]:
            self._create_parameter_row(params_frame, "n_participants", "Number of Participants", 100, "Number of participants to simulate")
        
        if self.test_name == "Threshold Insensitivity":
            self._create_parameter_row(params_frame, "n_drug_conditions", "Drug Conditions", 5, "Number of drug conditions to test")
        
        if self.test_name == "Cross-Species Validation":
            self._create_parameter_row(params_frame, "n_species", "Number of Species", 3, "Number of species to compare")
        
        if self.test_name == "Clinical Biomarkers":
            self._create_parameter_row(params_frame, "clinical_condition", "Clinical Condition", "depression", "Target clinical condition")
        
        if self.test_name == "Threshold Detection":
            self._create_parameter_row(params_frame, "modality", "Modality", "visual", "Sensory modality for threshold testing")
            self._create_parameter_row(params_frame, "method", "Method", "adaptive_staircase", "Threshold estimation method")
    
    def _create_parameter_row(self, parent_frame, param_name: str, label: str, default_value, tooltip: str):
    ...
    def _setup_test_parameters(self):
        ...
        if self.test_name == "Clinical Biomarkers":
            ...
            conditions = ["depression", "anxiety", "schizophrenia", "bipolar", "adhd", "autism"]
    ...
    def _validate_parameters(self) -> bool:
        ...
        try:
            ...
            if n_trials < 1 or n_trials > 100000:
                messagebox.showerror("Invalid Parameters", "Number of trials must be between 1 and 100000")
                return False
            ...
            if "n_participants" in self.test_vars:
                ...
                if n_participants < 1 or n_participants > 10000:
                    messagebox.showerror("Invalid Parameters", "Number of participants must be between 1 and 10000")
                    return False
    ...
    def _test_worker(self):
        ...
        try:
            ...
            for i in range(10):
                ...
                self._update_progress(completed_operations, total_operations, f"Running test... {i+1}/10")
    ...
    def _display_results(self):
        ...
        for key, value in results_dict.items():
            if isinstance(value, float):
                self.results_text.insert("end", f"{key.replace('_', ' ').title()}: {value:.3f}\n", "info")
            else:
        ...
        if "success_rate" in results_dict:
            ...
            if success_rate > 0.7:
                self.results_text.insert("end", f"Test Result: SUCCESS ({success_rate:.1%} success rate)\n", "success")
            elif success_rate > 0.4:
                self.results_text.insert("end", f"Test Result: PARTIAL ({success_rate:.1%} success rate)\n", "warning")
            else:
                self.results_text.insert("end", f"Test Result: FAILED ({success_rate:.1%} success rate)\n", "error")
        
        if "falsification_rate" in results_dict:
            falsification_rate = results_dict["falsification_rate"]
            self.results_text.insert("end", f"Falsification Rate: {falsification_rate:.1%}\n", "info")
        
        self.results_text.insert("end", f"Execution Time: {results_dict.get('execution_time', 'N/A')} seconds\n", "info")
        self.results_text.insert("end", f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n", "info")
    ...
    def _export_json(self, file_path: str):
        ...
        export_data = {
            ...
            "results": self.test_results.to_dict() if hasattr(self.test_results, 'to_dict') else self.test_results
        }
...
def create_test_execution_panel(parent, test_name: str, controller: Optional[MainApplicationController] = None,
                              progress_callback: Optional[Callable] = None, log_callback: Optional[Callable] = None,
                              results_callback: Optional[Callable] = None) -> TestExecutionPanel:
    """
    ...
    """
    return TestExecutionPanel(parent, test_name, controller, progress_callback, log_callback, results_callback)
