...
logger = get_logger("main_gui_controller") if 'get_logger' in globals() else logging.getLogger("main_gui_controller")
...
class MainGUIController:
    ...
    def _setup_logging_panel_callbacks(self):
        ...
        if self.logging_panel:
            self.logging_panel.set_log_added_callback(
                self.event_handlers['log_added']
            )
    ...
    def _setup_test_panel_callbacks(self, test_name: str, panel: TestExecutionPanel):
        ...
        panel.set_test_completed_callback(
            self._on_test_completed
        )
        panel.set_test_failed_callback(
            self._on_test_failed
        )
        panel.set_progress_updated_callback(
    ...
    def _on_progress_updated(self, test_name: str, progress: float, message: str):
        ...
        self._broadcast_event("progress_updated", {
            "test_name": test_name,
            "progress": progress,
            "message": message
        })
    ...
    def export_all_results(self, file_path: str) -> bool:
        ...
        try:
            export_data = {
                ...
                "log_statistics": self.logging_panel.get_log_statistics() if self.logging_panel else {}
            }
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "running_tests": sum(1 for panel in self.test_panels.values() if panel.is_test_running()),
        ...
        try:
            ...
            for test_name, panel in self.test_panels.items():
                if panel.is_test_running():
                    validation_results["warnings"].append(f"Test {test_name} is still running")
            
            # Check results panel
            if self.results_panel:
                results_data = self.results_panel.get_results_data()
                if len(results_data) > 1000:
                    validation_results["warnings"].append("Large amount of results data may affect performance")
            
            # Check logging panel
            if self.logging_panel:
                stats = self.logging_panel.get_log_statistics()
                if stats.get('ERROR', 0) > 10:
                    validation_results["warnings"].append("High number of error logs detected")
