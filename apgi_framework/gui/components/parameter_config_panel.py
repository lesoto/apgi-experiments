...
logger = get_logger("parameter_config_panel") if 'get_logger' in globals() else logging.getLogger("parameter_config_panel")
...
class ParameterConfigPanel(ctk.CTkFrame):
    ...
    def _create_experimental_config_section(self):
        ...
        for param, label, default, tooltip, valid_range, param_type in exp_params:
            self._create_parameter_row(
                exp_frame, param, label, default, tooltip, valid_range, "exp", param_type
            )
    
    def _create_parameter_row(self, parent_frame, param_name: str, label: str, 
                            default_value, tooltip: str, valid_range: Optional[tuple],
                            section: str, param_type: type = float):
        """
    ...
    def _create_control_buttons(self):
        ...
        validate_btn = ctk.CTkButton(
            control_frame,
            text="Validate All",
            command=self._validate_all_parameters
        )
        ...
        if param_name in validation_ranges:
            ...
            if min_val <= value <= max_val:
                ...
                self._set_validation_indicator(
                    param_name, False, f"Value {value} outside range [{min_val}, {max_val}]"
                )
        else:
        ...
        try:
            ...
            if self.validator:
                ...
                if result.is_valid:
                    ...
                    messagebox.showwarning("Validation", f"APGI parameter issues:\n{result.get_message()}")
            else:
                messagebox.showinfo("Validation", "Basic validation passed (validator not available)")
            ...
            if validation_errors:
                messagebox.showwarning("Validation", f"Experimental parameter issues:\n" + "\n".join(validation_errors))
            else:
...
def create_parameter_config_panel(parent, config_manager: Optional[ConfigManager] = None) -> ParameterConfigPanel:
    """
