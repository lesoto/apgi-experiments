...
class ResultsVisualizationPanel(ctk.CTkFrame):
    ...
    def _create_control_section(self):
        ...
        self.view_mode_combo = ctk.CTkOptionMenu(
            ...
            values=["Overview", "Statistical Details", "Time Series", "Comparison", "Neural Signatures"],
            width=15,
        )
    ...
    def _clear_data(self):
        ...
        if messagebox.askyesno("Clear Data", "Clear all visualization data? This cannot be undone."):
            self.results_data = []
    ...
    def _show_empty_plots(self):
        ...
        for i, ax in enumerate([self.ax1, self.ax2, self.ax3, self.ax4]):
            ax.text(
                ...
                fontsize=12, color=self.plot_colors['neutral']
            )
            ax.set_xticks([])
            ax.set_yticks([])
            
            # Set appropriate titles
            titles = ["Falsification Rates", "Effect Sizes", "P-values", "Statistical Power"]
    ...
    def _plot_falsification_rates(self, test_types: List[str], falsified: List[bool]):
        ...
        colors = [self.plot_colors['accent'] if r > 0.5 else self.plot_colors['primary'] 
        ...
        for i, (bar, count) in enumerate(zip(bars, test_counts)):
            ...
            self.ax1.text(bar.get_x() + bar.get_width() / 2.0, height,
    ...
    def _plot_effect_sizes(self, effect_sizes: List[float]):
        ...
        if effect_sizes:
            self.ax2.hist(effect_sizes, bins=min(10, len(effect_sizes)), 
                        alpha=0.7, color=self.plot_colors['secondary'], edgecolor="black")
            
            # Reference lines
            self.ax2.axvline(x=0.5, color=self.plot_colors['warning'], linestyle="--", 
                            label="Medium effect (0.5)", linewidth=2)
            self.ax2.axvline(x=0.8, color=self.plot_colors['accent'], linestyle="--", 
    ...
    def _plot_p_values(self, p_values: List[float]):
        ...
        if p_values:
            ...
            self.ax3.hist(p_values, bins=min(10, len(p_values)), 
                        alpha=0.7, color=self.plot_colors['neutral'], edgecolor="black")
            
            # Significance threshold
            self.ax3.axvline(x=0.05, color=self.plot_colors['accent'], linestyle="--", 
            ...
            self.ax3.text(0.95, 0.95, f"Significant: {significant}/{total} ({sig_rate:.1%})",
    ...
    def _plot_statistical_power(self, powers: List[float]):
        ...
        if powers:
            # Create histogram
            self.ax4.hist(powers, bins=min(10, len(powers)), 
                        alpha=0.7, color=self.plot_colors['primary'], edgecolor="black")
            
            # Adequate power threshold
            self.ax4.axvline(x=0.8, color=self.plot_colors['warning'], linestyle="--", 
                            label="Adequate power (0.8)", linewidth=2)
            ...
            self.ax4.text(0.95, 0.95, f"Adequate: {adequate}/{total} ({adequate_rate:.1%})",
                         transform=self.ax4.transAxes, ha="right", va="top",
                         bbox=dict(boxstyle="round", facecolor='white', alpha=0.8))
    ...
    def _show_comparison_plots(self):
        ...
        metrics = ["success_rate", "falsification_rate", "mean_effect_size", "mean_power"]
    ...
    def _plot_effect_sizes_with_ci(self, effect_sizes: List[float], confidence_intervals: List[List[float]]):
        ...
        if effect_sizes and confidence_intervals:
            ...
            self.ax1.errorbar(x_pos, effect_sizes, 
                            yerr=[np.array(effect_sizes) - np.array(ci_lower), 
                                  np.array(ci_upper) - np.array(effect_sizes)],
                            fmt='o', capsize=5, capthick=2, color=self.plot_colors['secondary'])
    ...
    def _plot_p_vs_effect_size(self, p_values: List[float], effect_sizes: List[float]):
        ...
        if p_values and effect_sizes:
            colors = [self.plot_colors['accent'] if p < 0.05 else self.plot_colors['neutral'] 
                     for p in p_values]
            
            self.ax2.scatter(effect_sizes, p_values, c=colors, alpha=0.7)
            self.ax2.axhline(y=0.05, color=self.plot_colors['accent'], linestyle='--', label="α = 0.05")
    ...
    def _plot_ci_widths(self, confidence_intervals: List[List[float]]):
        ...
        if confidence_intervals:
            ...
            self.ax3.hist(ci_widths, bins=min(10, len(ci_widths)), 
                          alpha=0.7, color=self.plot_colors['warning'], edgecolor="black")
    ...
    def _plot_power_analysis(self):
        ...
        self.ax4.axhline(y=0.8, color=self.plot_colors['warning'], linestyle='--', label="Adequate Power")
...
def create_results_visualization_panel(parent, results_callback: Optional[Callable] = None) -> ResultsVisualizationPanel:
    """
