...
class DetectionTask:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        mean_rt = np.mean([r["response_time_ms"] for r in self.results if r["detected"]]) if any(r["detected"] for r in self.results) else 0
...
class HeartbeatDetectionTask:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        logger.info(f"HeartbeatDetectionTask {self.task_id} completed: accuracy={mean_accuracy:.3f}")
...
class DualModalityOddballTask:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for trial in range(self.n_trials):
            ...
            response_data = self._generate_trial_response(
                trial_type, cardiac_phase
            )
        ...
        standard_accuracy = np.mean([r["correct"] for r in standard_trials]) if standard_trials else 0
        
        # Calculate P3b difference (oddball - standard)
        mean_p3b_oddball = np.mean([r["p3b_amplitude"] for r in oddball_trials]) if oddball_trials else 0
        mean_p3b_standard = np.mean([r["p3b_amplitude"] for r in standard_trials]) if standard_trials else 0
        ...
        logger.info(f"DualModalityOddballTask {self.task_id} completed: oddball_acc={oddball_accuracy:.3f}, p3b_diff={p3b_difference:.3f}")
