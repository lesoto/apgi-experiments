...
class CrossSpeciesValidator:
    ...
    def __init__(self, significance_threshold: float = 0.05):
        ...
        logger.info(
            f"CrossSpeciesValidator initialized with threshold {significance_threshold}"
        )
    ...
    def compare_signatures(
        ...
        correlation, p_value = self._compute_correlation(
            scaled_features1, scaled_features2
        )
        ...
        logger.info(
            f"Validation completed: score={validation_score:.3f}, p={p_value:.3f}"
        )
        return result
    ...
    def _extract_features_batch(
        self, signatures: List[CrossSpeciesSignature]
    ) -> np.ndarray:
    ...
    def _apply_species_scaling(
        self, features: np.ndarray, species: Species
    ) -> np.ndarray:
        ...
        for signature_type in signature_types:
            ...
            for i in range(len(species_list)):
                for j in range(i + 1, len(species_list)):
                    ...
                    try:
                        result = self.compare_signatures(
                            species1, species2, signature_type
                        )
    ...
    def generate_validation_report(
        ...
        for signature_type, validation_results in results.items():
            ...
            report.append(
                f"Mean validation score: {np.mean(scores):.3f} ± {np.std(scores):.3f}"
            )
            report.append("")
