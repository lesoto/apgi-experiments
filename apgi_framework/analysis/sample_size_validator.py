...
class SampleSizeValidator:
    ...
    def validate_soma_bias_sample_size(
        ...
        if not meets_minimum:
            ...
            recommendations.append(
                f"Increase sample size to at least {requirement.minimum_n + 1}"
            )
        ...
        if n_participants > 500:
            warnings_list.append(
                "Very large sample size - consider feasibility and cost"
            )
        ...
        return ValidationResult(
            ...
            and power_result.power >= self.power_threshold,
        )
    ...
    def validate_neural_signature_sample_size(
        ...
        if not meets_minimum:
            ...
            recommendations.append(
                f"Increase participants to at least {requirement.minimum_n}"
            )
        ...
        if n_trials_per_participant < 50:
            ...
            recommendations.append(
                "Consider increasing trials per participant to 50-100"
            )

        if power_result.power < self.power_threshold:
            warnings_list.append(
                f"Statistical power ({power_result.power:.3f}) below threshold"
            )
    ...
    def validate_falsification_sample_size(
        ...
        if not meets_minimum:
            ...
            recommendations.append(
                f"Increase sample size to at least {adjusted_minimum}"
            )
        ...
        return ValidationResult(
            ...
            recommendations=recommendations,
            meets_requirements=meets_minimum
            and power_result.power >= adjusted_power_threshold,
        )
    ...
    def validate_replication_sample_size(
        ...
        if power_result.power < 0.5:
            warnings_list.append(
                f"Individual lab power ({power_result.power:.3f}) very low"
            )
            recommendations.append(
        ...
        if meta_power_result.power < 0.9:
            warnings_list.append(
                f"Meta-analytic power ({meta_power_result.power:.3f}) below 0.9"
            )
    ...
    def generate_power_report(
        ...
        for config in study_configurations:
            ...
            if test_type == "soma_bias":
                result = self.validate_soma_bias_sample_size(
                    n_participants, effect_size
                )
                ...
                falsification_type = (
                    "primary" if "primary" in test_type else "secondary"
                )
                ...
                result = self.validate_replication_sample_size(
                    n_participants, n_labs, effect_size
                )
            else:
                # General validation
        ...
        overall_adequacy = all(
            result.meets_requirements for result in validation_results
        )
        ...
        power_summary = {
            ...
            "adequate_tests": sum(
                1 for result in validation_results if result.meets_requirements
            ),
        ...
        if power_summary["min_power"] < 0.5:
            recommendations.append(
                "Some tests have very low power - consider major design changes"
            )
    ...
    def _validate_general_sample_size(
        ...
        if not meets_minimum:
            ...
            recommendations.append(
                f"Increase sample size to at least {requirement.minimum_n}"
            )
