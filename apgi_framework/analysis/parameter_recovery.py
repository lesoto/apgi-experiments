...
class SyntheticDataGenerator:
    ...
    def generate_detection_task_data(
        ...
        p3b_amplitude[detected == 1] += self.rng.normal(
            0, noise_level * 0.5, np.sum(detected)
        )
...
class RecoveryAnalyzer:
    ...
    def analyze_recovery(
        ...
        rec_theta0_ci_lower = np.array(
            [r.theta0.credible_interval_95[0] for r in recovered_list]
        )
        rec_theta0_ci_upper = np.array(
            [r.theta0.credible_interval_95[1] for r in recovered_list]
        )

        rec_pi_i_ci_lower = np.array(
            [r.pi_i.credible_interval_95[0] for r in recovered_list]
        )
        rec_pi_i_ci_upper = np.array(
            [r.pi_i.credible_interval_95[1] for r in recovered_list]
        )

        rec_beta_ci_lower = np.array(
            [r.beta.credible_interval_95[0] for r in recovered_list]
        )
        rec_beta_ci_upper = np.array(
            [r.beta.credible_interval_95[1] for r in recovered_list]
        )
...
class ParameterRecoveryValidator:
    ...
    def run_validation(
        ...
        if verbose:
            self.logger.info(
                f"Successfully fit {len(recovered_list)}/{n_datasets} datasets"
            )
...
class ValidationReportGenerator:
    ...
    def generate_report(
        ...
        if recovery_results.passed:
            ...
            report.append(
                "  The pipeline can accurately recover ground-truth parameters."
            )
