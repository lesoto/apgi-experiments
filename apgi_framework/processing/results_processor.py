...
class ResultsProcessor:
    ...
    def __init__(self, output_dir: Optional[str] = None):
        ...
        self.output_dir = (
            Path(output_dir) if output_dir else Path("apgi_outputs/processed")
        )
        ...
        logger.info(
            f"ResultsProcessor initialized with output directory: {self.output_dir}"
        )
    ...
    def consolidate_results(
        ...
        try:
            ...
            primary_type = (
                experiment_types[0] if len(experiment_types) == 1 else "mixed"
            )
            ...
            consolidated = ProcessedResult(
                ...
                raw_data={
                    "consolidated_from": [r.result_id for r in processed_results]
                },
    ...
    def export_results(
        ...
        try:
            ...
            logger.info(
                f"Exporting {len(processed_results)} results to {export_format}"
            )

            if export_format == "csv":
                export_paths = self._export_to_csv(
                    processed_results, timestamp, include_metadata
                )
            elif export_format == "excel":
                export_paths = self._export_to_excel(
                    processed_results, timestamp, include_metadata
                )
            elif export_format == "json":
                export_paths = self._export_to_json(
                    processed_results, timestamp, include_metadata
                )
            ...
            if isinstance(trial_data, list):
                aggregated["by_trial"] = {
                    ...
                    "average_duration": np.mean(
                        [trial.get("duration", 0) for trial in trial_data]
                    ),
                }
            ...
            for condition_id, condition_results in condition_data.items():
                aggregated["by_condition"][condition_id] = {
                    ...
                    "participant_count": len(
                        set(condition_results.get("participants", []))
                    ),
                }
    ...
    def _assess_data_quality(self, raw_results: Dict[str, Any]) -> Dict[str, float]:
        ...
        def detect_outliers(obj):
            ...
            if isinstance(obj, dict):
                ...
                if numeric_array.size > 0 and np.issubdtype(
                    numeric_array.dtype, np.number
                ):
                    q1, q3 = np.percentile(numeric_array, [25, 75])
                    iqr = q3 - q1
                    lower_bound = q1 - 1.5 * iqr
                    upper_bound = q3 + 1.5 * iqr
                    outliers = (numeric_array < lower_bound) | (
                        numeric_array > upper_bound
                    )
        ...
        if "avg_sample_size" in quality_metrics:
            size_score = min(
                1.0,
                quality_metrics["avg_sample_size"]
                / self.quality_thresholds["min_sample_size"],
            )
        ...
        if "outlier_ratio" in quality_metrics:
            outlier_score = max(
                ...
                - (
                    ...
                    / self.quality_thresholds["outlier_ratio"]
                ),
            )
    ...
    def _generate_json_report(
        self, processed_result: ProcessedResult
    ) -> Dict[str, Any]:
    ...
    def _export_to_csv(
        ...
        for result in processed_results:
            ...
            stats_path = (
                self.output_dir
                / "exports"
                / f"{result.result_id}_stats_{timestamp}.csv"
            )
            ...
            quality_path = (
                self.output_dir
                / "exports"
                / f"{result.result_id}_quality_{timestamp}.csv"
            )
            ...
            if include_metadata:
                ...
                metadata_path = (
                    self.output_dir
                    / "exports"
                    / f"{result.result_id}_metadata_{timestamp}.csv"
                )
    ...
    def _export_to_excel(
        ...
        for result in processed_results:
            excel_path = (
                self.output_dir / "exports" / f"{result.result_id}_{timestamp}.xlsx"
            )
    ...
    def _export_to_json(
        ...
        for result in processed_results:
            json_path = (
                self.output_dir / "exports" / f"{result.result_id}_{timestamp}.json"
            )
    ...
    def _create_batch_summary(
        ...
        if processed_results:
            all_quality_scores = [
                r.quality_metrics.get("overall_quality_score", 0)
