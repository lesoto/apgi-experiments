...
class DataValidator:
    ...
    def validate_data_structure(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "apgi_parameters" in data:
            apgi_result = self._validate_apgi_parameters_structure(
        ...
        if "neural_signatures" in data:
            neural_result = self._validate_neural_signatures_structure(
    ...
    def validate_data_content(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "neural_signatures" in data:
            signature_result = self._validate_signature_values(
                data["neural_signatures"]
            )
    ...
    def calculate_quality_metrics(
        self, dataset: ExperimentalDataset
    ) -> Dict[str, float]:
        ...
        if dataset.data:
            ...
            for field_data in dataset.data.values():
                if isinstance(field_data, list):
                    ...
                    total_values += len(field_data)
                    missing_values += sum(
                        1 for item in field_data.values() if item is None
                    )
            ...
            if isinstance(signatures, list) and len(signatures) > 1:
                ...
                for sig_name in ["p3b_amplitude", "gamma_plv", "pci_value"]:
                    values = [
                        ...
                        if sig.get(sig_name) is not None
                    ]
