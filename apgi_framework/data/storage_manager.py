...
class StorageManager:
        ...
        try:
            ...
            if create_backup:
                try:
                    ...
                    self.logger.info(
                        f"Created backup {backup_info.backup_id} for {experiment_id}"
                    )
    ...
    def load_dataset(
        ...
        try:
            ...
            self.logger.error(
                f"Unexpected error loading dataset {experiment_id}: {str(e)}"
            )
            raise StorageError(
                f"Unexpected error loading dataset {experiment_id}: {str(e)}"
            )
        ...
        try:
            ...
            for exp_id in experiment_ids:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to load metadata for {exp_id}: {str(e)}"
                    )
                    continue
    ...
    def _apply_filters(
        ...
        if filters.experiment_ids:
            filtered = [
                m for m in filtered if m.experiment_id in filters.experiment_ids
            ]
        ...
        if filters.researcher:
            filtered = [
                m
                for m in filtered
                if filters.researcher.lower() in m.researcher.lower()
            ]
        ...
        if filters.institution:
            filtered = [
                m
                for m in filtered
                if filters.institution.lower() in m.institution.lower()
            ]
        ...
        if filters.tags:
            filtered = [
                m for m in filtered if any(tag in m.tags for tag in filters.tags)
            ]
        ...
        if filters.min_participants:
            filtered = [
                m for m in filtered if m.n_participants >= filters.min_participants
            ]

        if filters.max_participants:
            filtered = [
                m for m in filtered if m.n_participants <= filters.max_participants
            ]
        ...
        if filters.conditions:
            filtered = [
                m
                for m in filtered
                if any(cond in m.conditions for cond in filters.conditions)
            ]

        # Filter by validation status
        if filters.validation_status:
            filtered = [
                m for m in filtered if m.validation_status == filters.validation_status
            ]
            ...
            if create_version:
                version = DataVersion(
                    version_number=self._increment_version(
                        dataset.metadata.current_version
                    ),
                ...
                self.logger.info(
                    f"Created version {version.version_number} for {experiment_id}"
                )
    ...
    def create_backup(
        self, experiment_id: str, backup_type: str = "manual"
    ) -> BackupInfo:
        """
    ...
    def restore_from_backup(self, backup_id: str) -> str:
        ...
        try:
            ...
            if backup_info.backup_type == "full":
                ...
                restored_dataset = self._restore_incremental_backup(
                    backup_path, backup_info
                )
            else:
                raise StorageError(
                    f"Unsupported backup type: {backup_info.backup_type}"
                )
            ...
            self.logger.info(
                f"Restored dataset from backup {backup_id} as {new_experiment_id}"
            )
            ...
            for exp_id in experiment_ids:
                try:
                    ...
                    category_sizes[category] = (
                        category_sizes.get(category, 0.0) + size_mb
                    )
    ...
    def validate_all_datasets(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Validated {validation_summary['total_datasets']} datasets"
            )
