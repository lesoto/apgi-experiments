...
class PCICalculator:
    ...
    def generate_signature(
        self, target_pci: float, noise_level: float = 0.02
    ) -> PCISignature:
        """
        ...
        if final_pci > self.conscious_pci_threshold:
            ...
            state_classification = (
                "unconscious"
                if final_pci < self.unconscious_pci_threshold
                else "intermediate"
            )
    ...
    def _simulate_perturbation_response(
        ...
        n_regions = connectivity_matrix.shape[0]
        n_timepoints = int(
            self.default_perturbation_duration * self.default_sampling_rate
        )
    ...
    def _calculate_complexity_components(
        ...
        components["lempel_ziv_complexity"] = self._lempel_ziv_complexity(
        ...
        components["integration_measure"] = self._integration_measure(
            connectivity_matrix
        )

        # Differentiation measure (local specialization)
        components["differentiation_measure"] = self._differentiation_measure(
            perturbation_response
        )
    ...
    def validate_signature(self, signature: PCISignature) -> bool:
        ...
        if signature.is_conscious_threshold != (
            signature.pci_value > self.conscious_pci_threshold
        ):
            return False
