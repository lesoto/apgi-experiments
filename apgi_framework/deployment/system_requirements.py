...
class SystemRequirementsReport:
    ...
    def passed_count(self) -> int:
        ...
        return sum(
            1 for req in self.requirements if req.status == RequirementStatus.PASSED
        )
    ...
    def warning_count(self) -> int:
        """Number of warnings."""
        return sum(
            1 for req in self.requirements if req.status == RequirementStatus.WARNING
        )
    ...
    def failed_count(self) -> int:
        ...
        return sum(
            1 for req in self.requirements if req.status == RequirementStatus.FAILED
        )
    ...
    def can_run(self) -> bool:
        ...
        minimum_failed = any(
            ...
            and req.level == RequirementLevel.MINIMUM
...
class SystemRequirementsValidator:
    ...
    def validate_system(self) -> SystemRequirementsReport:
        ...
        report.requirements.extend(
            self._check_performance_requirements(report.performance_metrics)
        )
    ...
    def _check_hardware_requirements(self) -> List[SystemRequirement]:
        ...
        if ram_gb >= self.opt_ram_gb:
            ...
            ram_req.message = (
                "Minimum RAM met, but more recommended for optimal performance"
            )
        else:
            ram_req.status = RequirementStatus.FAILED
            ram_req.message = (
                f"Insufficient RAM: {ram_gb:.1f} GB < {self.min_ram_gb} GB minimum"
            )
        ...
        if cpu_cores >= self.opt_cpu_cores:
            ...
            cpu_req.message = (
                f"Insufficient CPU cores: {cpu_cores} < {self.min_cpu_cores} minimum"
            )
    ...
    def _check_software_requirements(self) -> List[SystemRequirement]:
        ...
        if python_version >= self.opt_python_version:
            ...
            python_req.message = (
                "Minimum Python version met, but newer version recommended"
            )
        else:
    ...
    def get_optimization_recommendations(
        self, report: SystemRequirementsReport
    ) -> List[str]:
        """
        ...
        for req in report.requirements:
            if req.status == RequirementStatus.FAILED:
                ...
                if req.name == "RAM":
                    ...
                    recommendations.append(
                        "More CPU cores would improve processing speed"
                    )
        ...
        if report.performance_metrics.get("cpu_usage_percent", 0) > 80:
            recommendations.append(
                "High CPU usage detected, close background applications"
            )

        if report.performance_metrics.get("memory_usage_percent", 0) > 80:
            recommendations.append(
                "High memory usage detected, close unnecessary applications"
            )
