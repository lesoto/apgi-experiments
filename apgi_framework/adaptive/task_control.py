...
class PrecisionTimer:
    ...
    def get_timing_statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'std_error_ms': (sum((e - sum(errors)/len(errors))**2 for e in errors) / len(errors))**0.5,
...
class ResponseCollector:
    ...
    def start_collection(self, 
                        ...
                        response_window_ms: float = 5000.0) -> None:
        """
        ...
        while not self.stop_collection.is_set():
            ...
            if elapsed_ms > 200 and not self.response_queue.qsize():  # Minimum 200ms reaction time
                ...
                if random.random() < 0.001:  # Low probability per poll
                    ...
                    reaction_time = (response_time - self.stimulus_onset_time).total_seconds() * 1000
    ...
    def collect_confidence_rating(self, 
                                 ...
                                 scale_min: float = 0.0,
                                 scale_max: float = 1.0) -> Optional[float]:
        """
...
class TaskStateMachine:
    ...
    def transition_to(self, new_state: TaskState, message: str = "") -> bool:
        ...
        logger.info(f"State transition: {self.previous_state.value} -> {new_state.value} ({message})")
...
class SessionConfiguration:
    ...
    def validate(self) -> bool:
        ...
        return (len(self.session_id) > 0 and
                len(self.participant_id) > 0 and
                len(self.tasks_to_run) > 0 and
                self.max_session_duration_min > 0)
...
class SessionManager:
    ...
    def configure_session(self, config: SessionConfiguration) -> bool:
        ...
        logger.info(f"Session configured: {config.session_id} for participant {config.participant_id}")
        return True
    ...
    def complete_current_task(self) -> bool:
        ...
        if self.current_task_index < len(self.current_session.tasks_to_run):
            self.state_machine.transition_to(TaskState.READY, f"Task {task_name} completed, preparing next")
            
            # Schedule inter-task interval
            if self.current_session.inter_task_interval_s > 0:
                logger.info(f"Inter-task interval: {self.current_session.inter_task_interval_s:.1f}s")
    ...
    def _start_data_backup(self) -> None:
        ...
        if self.current_session and self.current_session.backup_interval_min > 0:
            ...
            logger.info(f"Data backup started: {self.current_session.backup_interval_min:.1f} min intervals")
