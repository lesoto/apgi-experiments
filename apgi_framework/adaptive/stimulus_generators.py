...
class StimulusParameters:
    ...
    def validate(self) -> bool:
        ...
        return (0.0 <= self.intensity <= 1.0 and 
                self.duration_ms > 0)
...
class GaborParameters(StimulusParameters):
    ...
    def validate(self) -> bool:
        ...
        return (super().validate() and
                0.0 <= self.contrast <= 1.0 and
                self.spatial_frequency > 0 and
                0.0 <= self.orientation < 360.0 and
                self.size_degrees > 0)
...
class ToneParameters(StimulusParameters):
    ...
    def validate(self) -> bool:
        ...
        return (super().validate() and
                20.0 <= self.frequency_hz <= 20000.0 and
                0.0 <= self.amplitude <= 1.0 and
                self.onset_ramp_ms >= 0 and
                self.offset_ramp_ms >= 0)
...
class CO2PuffParameters(StimulusParameters):
    ...
    def validate(self) -> bool:
        ...
        return (super().validate() and
                0.0 <= self.co2_concentration <= 100.0 and
                self.flow_rate > 0 and
                self.pressure > 0 and
                self.duration_ms <= self.max_duration_ms and
                self.nozzle_distance_cm > 0)
...
class GaborPatchGenerator(StimulusGenerator):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            degrees_per_pixel = np.degrees(np.arctan(screen_width_cm / self.viewing_distance_cm)) / self.screen_width
    ...
    def _create_gabor_patch(self, parameters: GaborParameters) -> np.ndarray:
        ...
        x = np.linspace(-parameters.size_degrees/2, parameters.size_degrees/2, size_pixels)
        y = np.linspace(-parameters.size_degrees/2, parameters.size_degrees/2, size_pixels)
        X, Y = np.meshgrid(x, y)
    ...
    def generate_stimulus(self, parameters: GaborParameters) -> bool:
        ...
        try:
            ...
            logger.debug(f"Presenting Gabor patch: contrast={parameters.contrast:.3f}, "
...
class ToneGenerator(StimulusGenerator):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Audio system initialized: {self.sample_rate} Hz, buffer {self.buffer_size}")
            return True
    ...
    def generate_stimulus(self, parameters: ToneParameters) -> bool:
        ...
        try:
            ...
            if parameters.sync_to_heartbeat:
                ...
                presentation_time = r_peak_time + timedelta(milliseconds=parameters.heartbeat_delay_ms)
            ...
            logger.debug(f"Presenting tone: {parameters.frequency_hz:.1f} Hz, "
...
class CO2PuffController(StimulusGenerator):
    ...
    def __init__(self, 
                 generator_id: str = "co2_controller",
    ...
    def _check_safety_constraints(self, parameters: CO2PuffParameters) -> bool:
        ...
        if self.last_puff_time is not None:
            ...
            if elapsed_ms < parameters.min_interval_ms:
                logger.error(f"Minimum interval not met: {elapsed_ms:.1f}ms < {parameters.min_interval_ms:.1f}ms")
                return False
        
        # Check duration limits
        if parameters.duration_ms > parameters.max_duration_ms:
            logger.error(f"Duration exceeds safety limit: {parameters.duration_ms:.1f}ms > {parameters.max_duration_ms:.1f}ms")
            return False
    ...
    def generate_stimulus(self, parameters: CO2PuffParameters) -> bool:
        ...
        try:
            ...
            logger.debug(f"Delivering CO2 puff: {parameters.co2_concentration:.1f}% CO2, "
            ...
            logger.info(f"CO2 puff delivered successfully (session puff #{self.total_puffs_session})")
            return True
    ...
    def _monitor_cardiac_signal(self) -> None:
        ...
        while not self.stop_monitoring.is_set():
            try:
                ...
                if (self.last_r_peak is None or 
                    (current_time - self.last_r_peak).total_seconds() * 1000 >= self.refractory_period_ms):
    ...
    def wait_for_r_peak(self, timeout_ms: float = 2000.0) -> Optional[datetime]:
        ...
        while (datetime.now() - start_time).total_seconds() * 1000 < timeout_ms:
            if self.last_r_peak != last_known_r_peak:
                logger.debug(f"R-peak detected for synchronization: {self.last_r_peak.isoformat()}")
