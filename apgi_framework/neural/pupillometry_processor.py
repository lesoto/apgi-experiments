    ...
    def process_trial(
        ...
        if self.blink_detector:
            blinks = self.blink_detector.detect_blinks(
                pupil_data, timestamps, confidence
            )
        ...
        baseline_start_idx = np.searchsorted(
            timestamps, stimulus_time + baseline_window[0]
        )
        baseline_end_idx = np.searchsorted(
            timestamps, stimulus_time + baseline_window[1]
        )
...
class PupilFeatureExtractor:
    ...
    def extract_window_features(
        ...
        percent_change = (
            ((peak_dilation - baseline) / baseline * 100) if baseline > 0 else 0.0
        )
...
class PupilMetricsCalculator:
    ...
    def compute_response_metrics(
        ...
        window_start_idx = np.searchsorted(
            timestamps, stimulus_time + response_window[0]
        )
        ...
        onset_indices = np.where(window_data > threshold)[0]
        latency = (
            (window_times[onset_indices[0]] - stimulus_time)
            if len(onset_indices) > 0
            else 0.0
        )
...
class PupilQualityScorer:
    ...
    def assess_quality(
        ...
        tracking_confidence = (
            np.mean(valid_confidence) if len(valid_confidence) > 0 else 0.0
        )
