...
class RespirationMonitor:
    ...
    def detect_breath_cycles(
        ...
        smoothed = np.convolve(
            resp_signal, np.ones(window_size) / window_size, mode="same"
        )
            ...
            while self.is_streaming:
                ...
                if self.config.enable_ecg:
                    if data_sources and SignalType.ECG in data_sources:
                        ...
                        ecg_value = (
                            np.sin(2 * np.pi * 1.2 * current_time)
                            + np.random.randn() * 0.1
                        )
                    ...
                    if len(self.ecg_buffer) >= 100:
                        ...
                        if len(r_peaks) > 0:
                            rr_intervals = self.heart_rate_monitor.compute_rr_intervals(
                                r_peaks
                            )
                            if len(rr_intervals) > 0:
                                sample.heart_rate = (
                                    self.heart_rate_monitor.compute_heart_rate(
                                        rr_intervals
                                    )
                                )
                                sample.rr_interval = (
                ...
                if self.config.enable_scr:
                    ...
                    if len(self.scr_buffer) >= 100:
                        ...
                        tonic, phasic = self.scr_monitor.decompose_signal(
                            scr_data, timestamps
                        )
                ...
                if self.config.enable_respiration:
                    if data_sources and SignalType.RESP in data_sources:
                        ...
                        resp_value = (
                            np.sin(2 * np.pi * 0.25 * current_time)
                    ...
                    if len(self.resp_buffer) >= 100:
                        ...
                        if cycles:
                            sample.respiration_rate = (
                                self.respiration_monitor.compute_respiration_rate(
                                    cycles
                                )
                            )
                            sample.respiration_phase = (
                                self.respiration_monitor.get_current_phase(
    ...
    def export_data(self, filename: str, format: str = "numpy") -> None:
        ...
        if format == "numpy":
            data_dict = {
                ...
                "heart_rate": np.array(
                    [
                        s.heart_rate if s.heart_rate is not None else np.nan
                        for s in samples
                    ]
                ),
                "rr_interval": np.array(
                    [
                        s.rr_interval if s.rr_interval is not None else np.nan
                        for s in samples
                    ]
                ),
                "scr_level": np.array(
                    [
                        s.scr_level if s.scr_level is not None else np.nan
                        for s in samples
                    ]
                ),
                "scr_response": np.array(
                    [
                        s.scr_response if s.scr_response is not None else np.nan
                        for s in samples
                    ]
                ),
                ...
                "temperature": np.array(
                    [
                        s.temperature if s.temperature is not None else np.nan
                        for s in samples
                    ]
                ),
                ...
                for s in samples:
                    writer.writerow(
                        [
                            ...
                            (
                                s.respiration_phase.value
                                if s.respiration_phase is not None
                                else ""
                            ),
                            (
                                s.respiration_amplitude
