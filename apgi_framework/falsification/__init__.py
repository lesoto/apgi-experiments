...
class ConsciousnessWithoutIgnitionTest:
    ...
    def run_test(self, n_trials: int = 100, noise_level: float = 0.1) -> dict:
        ...
        for trial in range(n_trials):
            ...
            forced_choice_acc = np.random.beta(8, 4) if subjective_conscious else np.random.beta(4, 8)
            confidence = forced_choice_acc + np.random.normal(0, 0.1)
            confidence = np.clip(confidence, 0, 1)
            
            full_consciousness = (subjective_conscious and 
                                forced_choice_acc > self.consciousness_thresholds['forced_choice_accuracy'] and
                                confidence > self.consciousness_thresholds['confidence_rating'])
        ...
        contingency_table = [[consciousness_without_ignition, both_present - consciousness_without_ignition],
...
class ThresholdInsensitivityTest:
    ...
    def run_test(self, n_trials: int = 100, threshold_variations: float = 0.2) -> dict:
        ...
        # Analyze sensitivity
        falsification_rates = [r['results']['falsification_rate'] for r in results_by_config]
        sensitivity = np.std(falsification_rates) / (np.mean(falsification_rates) + 1e-8)
    ...
    def _run_configuration_simulation(self, n_trials: int, thresholds: dict) -> dict:
        ...
        for trial in range(n_trials):
            ...
            signatures_met = (
                p3b_amp > thresholds['p3b_amplitude'] and
                gamma_plv > thresholds['gamma_plv'] and
                bold_act > thresholds['bold_activation'] and
                pci_val > thresholds['pci_value']
            )
            ...
            consciousness_met = (
                forced_choice_acc > thresholds['forced_choice_accuracy'] and
                confidence > thresholds['confidence_rating']
            )
...
class SomaBiasTest:
    ...
    def run_test(self, n_trials: int = 100, bias_strength: float = 0.3) -> dict:
        ...
        for bias_level in self.soma_bias_levels:
            ...
            results_by_bias.append({
                'bias_level': bias_level,
                'results': bias_results
            })
        
        # Analyze bias effects
        falsification_rates = [r['results']['falsification_rate'] for r in results_by_bias]
        ...
        positive_bias_rates = [r['results']['falsification_rate'] for r in results_by_bias if r['bias_level'] > 0]
        negative_bias_rates = [r['results']['falsification_rate'] for r in results_by_bias if r['bias_level'] < 0]
        
        asymmetry_test = None
        asymmetry_p = None
        if positive_bias_rates and negative_bias_rates:
            asymmetry_test, asymmetry_p = stats.ttest_ind(positive_bias_rates, negative_bias_rates)
        
        # Determine if framework is falsified
        falsified = (abs(correlation) > 0.7 and p_value < 0.05) or (asymmetry_p and asymmetry_p < 0.05)
        ...
        return {
            ...
            "bias_analysis": {
                ...
                "positive_bias_mean": np.mean(positive_bias_rates) if positive_bias_rates else None,
                "negative_bias_mean": np.mean(negative_bias_rates) if negative_bias_rates else None
            },
            "framework_falsified": falsified,
    ...
    def _run_bias_simulation(self, n_trials: int, bias_level: float, bias_strength: float) -> dict:
        ...
        for trial in range(n_trials):
            ...
            forced_choice_acc = np.random.beta(8 + bias_consciousness_effect * 4, 
                                             4 - bias_consciousness_effect * 2)
            confidence = forced_choice_acc + np.random.normal(0, 0.1)
            confidence = np.clip(confidence, 0, 1)
            
            consciousness_met = (
                forced_choice_acc > 0.6 and
                confidence > 0.5
            )
