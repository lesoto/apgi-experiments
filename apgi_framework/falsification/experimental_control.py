...
class MotorResponse:
    ...
    def __post_init__(self):
        ...
        if self.reaction_time < 0:
            raise ValidationError(
                f"Reaction time must be positive, got {self.reaction_time}"
            )
...
class VerbalResponse:
    ...
    def __post_init__(self):
        ...
        if self.response_latency < 0:
            raise ValidationError(
                f"Response latency must be positive, got {self.response_latency}"
            )
...
class MetacognitiveAssessment:
    ...
    confidence_accuracy_correlation: (
        float  # Correlation between confidence and accuracy
    )
...
class ExperimentalControlValidator:
    ...
    def validate_complete_experimental_controls(
        ...
        task_adequate, task_details = self.validate_task_comprehension(
            task_comprehension
        )
        stimulus_valid, stimulus_details = self.validate_stimulus_presentation(
            stimulus_validation
        )
