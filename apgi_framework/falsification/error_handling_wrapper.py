...
def setup_detailed_logging():
    ...
    try:
        ...
        log_file = os.path.join(
            log_dir, f'falsification_tests_{datetime.now().strftime("%Y%m%d")}.log'
        )
            ...
            try:
                ...
                if validate_params and kwargs:
                    try:
                        ...
                        if log_errors:
                            logger.error(
                                f"Parameter validation failed for {func_name}: {str(e)}"
                            )
                ...
                if enable_retry:
                    logger.debug(
                        f"Executing {func_name} with retry enabled (max_retries={max_retries})"
                    )
                ...
                logger.info(
                    f"Attempting automatic recovery for simulation error in {func_name}"
                )
                ...
                raise APGIFrameworkError(
                    f"Memory error in {func_name}: Insufficient memory. Try reducing data size."
                ) from e
                ...
                wrapped_error = APGIFrameworkError(
...
def _validate_test_parameters(func_name: str, params: Dict[str, Any]):
    ...
    if "n_participants" in params:
        ...
        if not isinstance(n_participants, int) or n_participants <= 0:
            raise ValidationError(
                f"n_participants must be a positive integer, got {n_participants}"
            )
...
class ErrorHandlingTestWrapper:
    ...
    def __init__(self, test_controller: Any):
        ...
        logger.info(
            f"Initialized ErrorHandlingTestWrapper for {type(test_controller).__name__}"
        )
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if callable(attr) and name.startswith("run_"):
            ...
            @with_error_handling(
                validate_params=True, enable_retry=True, log_errors=True, max_retries=3
            )
            def wrapped_method(*args, **kwargs):
                ...
                while attempt_count < max_attempts:
                    try:
                        attempt_count += 1
                        logger.info(
                            f"Executing {name} (attempt {attempt_count}/{max_attempts})"
                        )
                        ...
                        # Log successful execution
                        execution_time = (
                            datetime.now() - method_start_time
                        ).total_seconds()
                        logger.info(
                            f"{name} completed successfully in {execution_time:.2f}s"
                        )
                        ...
                        logger.error(
                            f"Error in {name} (attempt {attempt_count}/{max_attempts}): {str(e)}"
                        )

                        # Determine if error is transient and should be retried
                        if attempt_count < max_attempts:
                            is_transient = handle_transient_failure(
                                e, error_entry, attempt_count, max_attempts
                            )
                ...
                raise APGIFrameworkError(
                    f"Max attempts ({max_attempts}) exceeded for {name}"
                )
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_problematic_method": (
                max(error_methods.items(), key=lambda x: x[1])[0]
                if error_methods
                else None
            ),
        }
...
def create_safe_test_controller(
    controller_class: type, *args, **kwargs
    ...
    try:
        ...
        logger.error(
            f"Failed to create test controller {controller_class.__name__}: {str(e)}"
        )
        ...
        error_report = create_error_report(
            f"create_{controller_class.__name__}", e, context
        )
        logger.debug(f"Error report:\n{error_report}")

        raise ConfigurationError(
            f"Cannot initialize test controller {controller_class.__name__}: {str(e)}"
        ) from e
...
def log_test_execution(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            duration = (end_time - start_time).total_seconds()
            logger.info(
                f"✓ Test '{test_name}' completed successfully in {duration:.2f}s"
            )
...
def create_error_report(
    test_name: str, error: Exception, context: Dict[str, Any]
) -> str:
    """
...
def handle_transient_failure(
    error: Exception, context: Dict[str, Any], attempt: int, max_attempts: int
) -> bool:
    """
    ...
    if is_transient or is_transient_type:
        logger.info(
            f"Transient failure detected (attempt {attempt}/{max_attempts}): {type(error).__name__}"
        )
