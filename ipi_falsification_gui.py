...
class ParameterConfigPanel(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        for i, (param, label, default, tooltip) in enumerate(ipi_params):
            # Label
            label_widget = ttk.Label(ipi_frame, text=f"{label}:")
            ...
            indicator = ttk.Label(ipi_frame, text="✓", foreground="green", width=2)
    ...
    def _setup_validation(self):
        ...
        # Add trace callbacks for IPI parameters
        for param, var in self.param_vars.items():
            var.trace_add("write", lambda *args, p=param: self._validate_ipi_parameter(p))
    ...
    def _validate_ipi_parameter(self, param_name):
        """Validate a single IPI parameter in real-time."""
        try:
            value = self.param_vars[param_name].get()
            result = self.validator.validate_ipi_parameters(**{param_name: value})
    ...
    def _validate_all_parameters(self):
        ...
        try:
            ...
            ipi_params = {param: var.get() for param, var in self.param_vars.items()}
            exp_config = {}
            neural_thresholds = {}
