...
pipeline for IPI framework validation.
"""
...
def example_basic_threshold_estimation():
    ...
    if visual_result:
        logger.info(f"Visual threshold: {visual_result.conscious_threshold.threshold:.4f}")
    ...
    if auditory_result:
        logger.info(f"Auditory threshold: {auditory_result.conscious_threshold.threshold:.4f}")
...
def example_neural_validation():
    ...
    if validation_result:
        ...
        logger.info(f"Trials rejected (artifacts): {validation_result.n_trials_rejected}")
        
        logger.info("\n--- Neural Signatures ---")
        logger.info(f"P3b-threshold correlation: {validation_result.p3b_amplitude_threshold_correlation:.3f}")
        logger.info(f"Gamma-threshold correlation: {validation_result.gamma_power_threshold_correlation:.3f}")
        logger.info(f"P3b stochastic appearance: {validation_result.p3b_stochastic_appearance_detected}")
        logger.info(f"P3b detection rate near threshold: {validation_result.p3b_detection_rate_near_threshold:.2f}")
        
        logger.info("\n--- Neural-Behavioral Correspondence ---")
        logger.info(f"P3b predicts detection: {validation_result.p3b_predicts_detection:.3f}")
        logger.info(f"Gamma predicts detection: {validation_result.gamma_predicts_detection:.3f}")
        
        logger.info("\n--- Quality Metrics ---")
        logger.info(f"Signal quality: {validation_result.overall_signal_quality:.2f}")
        logger.info(f"Validation confidence: {validation_result.validation_confidence:.2f}")
        
        # Validate against IPI predictions
        logger.info("\n--- IPI Framework Validation ---")
...
def example_test_retest_reliability():
    ...
    if result_s1:
        logger.info(f"Session 1 threshold: {result_s1.conscious_threshold.threshold:.4f}")
    ...
    if result_s2:
        logger.info(f"Session 2 threshold: {result_s2.conscious_threshold.threshold:.4f}")
...
def example_complete_validation_battery():
    ...
    for modality_name, config in modalities:
        ...
        if result:
            ...
            logger.info(f"P3b correlation: {result.p3b_amplitude_threshold_correlation:.3f}")
            logger.info(f"Gamma correlation: {result.gamma_power_threshold_correlation:.3f}")
...
if __name__ == "__main__":
    ...
    logger.info("IPI Framework - Priority 1 Threshold Estimation Examples")
    logger.info("=" * 60)
