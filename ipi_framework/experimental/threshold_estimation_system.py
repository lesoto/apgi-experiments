...
from .adaptive_staircase import (
    AdaptiveStaircase, StaircaseParameters, StaircaseType,
    ThresholdEstimate, CrossModalThresholdNormalizer,
    create_staircase
)
from .multi_modal_task_manager import (
    MultiModalTaskManager, ModalityType, TaskParadigm,
    TrialConfiguration, TrialResult
)
from ..adaptive.stimulus_generators import (
    GaborParameters, ToneParameters, CO2PuffParameters
)
...
class ModalityThresholdConfig:
    ...
    def validate(self) -> bool:
        ...
        if self.modality == ModalityType.INTEROCEPTIVE and self.base_interoceptive_params is None:
...
class ThresholdEstimationResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'detection_threshold': self.detection_threshold.to_dict() if self.detection_threshold else None,
            'conscious_threshold': self.conscious_threshold.to_dict() if self.conscious_threshold else None,
...
class ThresholdEstimationProtocol:
    ...
    def __init__(self,
        ...
        logger.info(f"Initialized ThresholdEstimationProtocol for participant {participant_id}")
    ...
    def run_threshold_estimation(self,
                                config: ModalityThresholdConfig) -> Optional[ThresholdEstimationResult]:
        """
        ...
        for block_num in range(config.n_blocks):
            ...
            for trial_num in range(config.n_trials_per_block):
                ...
                if config.use_awareness_probe and config.threshold_type == ThresholdType.CONSCIOUS_ACCESS:
                    ...
                    conscious_response = self._probe_conscious_awareness(
                        trial_result,
                        config.awareness_probe_delay_ms
                    )
        ...
        logger.info(f"Threshold estimation completed: {threshold_estimate.threshold:.4f} "
                   f"(±{threshold_estimate.std_error:.4f})")
        
        return result
    
    def _create_trial_config(self,
                            config: ModalityThresholdConfig,
                            trial_number: int,
                            intensity: float) -> TrialConfiguration:
        ...
        elif config.modality == ModalityType.INTEROCEPTIVE and config.base_interoceptive_params:
        ...
        return TrialConfiguration(
            ...
            metadata={'intensity': intensity, 'threshold_type': config.threshold_type.value}
        )
    
    def _probe_conscious_awareness(self,
    ...
    def calculate_test_retest_reliability(self,
                                         previous_result: ThresholdEstimationResult,
                                         current_result: ThresholdEstimationResult) -> Optional[float]:
        """
    ...
    def save_results(self, output_dir: str) -> bool:
        ...
        try:
            ...
            summary = {
                ...
                'session_duration_minutes': (datetime.now() - self.session_start_time).total_seconds() / 60.0
                    if self.session_start_time else None
            }
            
            summary_file = output_path / f"threshold_summary_{self.participant_id}_{self.session_id}.json"
