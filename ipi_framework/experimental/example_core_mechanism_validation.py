...
surprise accumulation dynamics analyzer to validate IPI framework predictions.
"""

import numpy as np
from typing import List

from ipi_framework.experimental.neuromodulatory_blockade import (
    NeuromodulatoryBlockadeSimulator,
    NeuromodulatorEffect
)
from ipi_framework.experimental.surprise_accumulation_dynamics import (
    SurpriseAccumulationAnalyzer,
    TrialSurpriseEstimate
)
from ipi_framework.neural.erp_analysis import ERPAnalysis, ERPComponents
from ipi_framework.core.equation import IPIEquation
...
def example_neuromodulatory_blockade():
    ...
    print(f"  ✓ Early ERP preservation (<10% change): {result.meets_early_preservation_prediction}")
    print(f"  ✓ Emotional selectivity: {result.meets_emotional_selectivity_prediction}")
    
    all_met = (result.meets_p3b_reduction_prediction and
               result.meets_early_preservation_prediction and
               result.meets_emotional_selectivity_prediction)
    
    if all_met:
        print("\n✓ ALL PREDICTIONS MET - IPI framework validated!")
    else:
...
def example_surprise_accumulation_dynamics():
    ...
    print(f"{'Trial':<10} {'Est. S_t':<10} {'Threshold':<10} {'P(ignition)':<12} {'Actual':<8} {'RT (ms)':<10}")
    print("-" * 70)
    
    for i in range(min(10, len(result.trial_estimates))):
        est = result.trial_estimates[i]
        print(f"{est.trial_id:<10} {est.estimated_surprise:<10.2f} {est.estimated_threshold:<10.2f} "
...
def example_combined_validation():
    ...
    dynamics, providing comprehensive validation of IPI framework predictions.
    """
    print("\n\n")
    print("=" * 70)
    ...
    print(f"Baseline prediction accuracy: {baseline_result.ignition_prediction_accuracy:.3f}")
    ...
    print(f"Blockade prediction accuracy: {blockade_result.ignition_prediction_accuracy:.3f}")
    print(f"Blockade AUC: {blockade_result.ignition_prediction_auc:.3f}")
    
    # Compare conditions
    print("\nCOMPARISON:")
    print(f"  Ignition rate baseline: {baseline_result.n_ignition_trials / baseline_result.n_trials:.2%}")
    print(f"  Ignition rate blockade: {blockade_result.n_ignition_trials / blockade_result.n_trials:.2%}")
    print(f"  Reduction: {(1 - blockade_result.n_ignition_trials / baseline_result.n_ignition_trials):.1%}")
...
if __name__ == "__main__":
    # Run all examples
    print("IPI FRAMEWORK - PRIORITY 2 CORE MECHANISM VALIDATION")
    print("=" * 70)
    print()
