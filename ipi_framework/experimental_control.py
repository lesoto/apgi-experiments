...
systems for the IPI Framework falsification testing. It ensures proper experimental
...
class ExperimentalControlManager:
    ...
    def validate_response_system(self, 
                               participant_id: str,
                               system_type: ResponseSystemType,
                               response_times: List[float],
                               accuracies: List[bool]) -> ResponseSystemValidation:
        """
        ...
        """
        logger.info(f"Validating {system_type.value} response system for participant {participant_id}")
    ...
    def verify_task_comprehension(self,
                                participant_id: str,
                                task_type: TaskType,
                                training_responses: List[bool],
                                instruction_clarity_rating: float) -> TaskComprehensionResult:
        """
        ...
        """
        logger.info(f"Verifying task comprehension for {task_type.value} task, participant {participant_id}")
        ...
        if len(training_responses) >= 10:
            ...
            windows = [training_responses[i:i+window_size] 
        ...
        comprehension_score = (performance_score + response_consistency + instruction_clarity_rating) / 3.0
        
        overall_comprehension = (sufficient_trials and 
                               performance_criterion_met and 
                               comprehension_score >= thresholds['minimum_score'])
    ...
    def generate_control_report(self,
                              ...
                              comprehension_results: List[TaskComprehensionResult]) -> ExperimentalControlReport:
        """
        ...
        """
        logger.info(f"Generating control report for participant {participant_id}, session {session_id}")
        ...
        if not response_validity:
            invalid_systems = [rv.system_type.value for rv in response_validations if not rv.is_intact]
            recommendations.append(f"Response system issues detected: {', '.join(invalid_systems)}")
        
        if not comprehension_validity:
            failed_tasks = [cr.task_type.value for cr in comprehension_results if not cr.performance_criterion_met]
...
class ParticipantSimulator:
    ...
    def simulate_response(self,
                         ...
                         correct_answer: Any,
                         response_options: List[Any]) -> SimulatedResponse:
        """
        ...
        performance = (self.profile.cognitive_ability * 
                      difficulty_modifier * 
                      fatigue_modifier * 
                      learning_modifier * 
                      attention_modifier)
        ...
        if is_correct:
            ...
            response_value = np.random.choice(incorrect_options) if incorrect_options else correct_answer
        ...
        rt_fatigue = rt_difficulty * (1.0 + self.fatigue_accumulation * 0.3)  # Fatigue slows responses
    ...
    def simulate_training_session(self,
                                task_difficulty: float,
                                n_trials: int,
                                correct_answers: List[Any],
                                response_options: List[List[Any]]) -> List[SimulatedResponse]:
        """
        ...
        """
        logger.info(f"Simulating training session: {n_trials} trials for {self.profile.participant_id}")
    ...
    def simulate_experimental_session(self,
        ...
        """
        logger.info(f"Simulating experimental session: {len(task_difficulties)} trials for {self.profile.participant_id}")
        
        responses = []
        
        for i, (difficulty, base_rt, correct, options) in enumerate(
            zip(task_difficulties, base_response_times, correct_answers, response_options)
        ):
            response = self.simulate_response(
    ...
    def _update_learning(self):
        ...
        if self.trial_count > 0:
            self.learning_progress = min(1.0, self.profile.learning_rate * np.log(1 + self.trial_count * 0.1))
    ...
    def get_performance_metrics(self, responses: List[SimulatedResponse]) -> Dict[str, float]:
        ...
        return {
            ...
            'consistency_score': 1.0 - (np.std(response_times) / np.mean(response_times)),
...
class ParticipantPopulationGenerator:
    ...
    @staticmethod
    def generate_population(n_participants: int, 
                          random_seed: Optional[int] = None) -> List[ParticipantProfile]:
        """
    ...
    @staticmethod
    def generate_clinical_population(n_participants: int,
                                   condition: str,
                                   random_seed: Optional[int] = None) -> List[ParticipantProfile]:
        """
...
class StimulusValidator:
    ...
    def validate_stimulus(self, stimulus: StimulusProperties) -> Tuple[bool, List[str]]:
        ...
        if stimulus.presentation_duration < min_duration:
            issues.append(f"Presentation duration too short: {stimulus.presentation_duration:.3f}s < {min_duration:.3f}s")
    ...
    def _validate_visual_stimulus(self, stimulus: StimulusProperties) -> List[str]:
        ...
        supraliminal_contrast = min_contrast * self.config['supraliminal_criteria']['visual_contrast_multiplier']
        
        if stimulus.contrast < supraliminal_contrast:
            issues.append(f"Visual contrast too low: {stimulus.contrast:.3f} < {supraliminal_contrast:.3f}")
    ...
    def _validate_auditory_stimulus(self, stimulus: StimulusProperties) -> List[str]:
        ...
        supraliminal_intensity = min_intensity * self.config['supraliminal_criteria']['intensity_multiplier']
        
        # Assuming luminance field is repurposed for intensity in auditory stimuli
        if stimulus.luminance < supraliminal_intensity:
            issues.append(f"Auditory intensity too low: {stimulus.luminance:.1f} < {supraliminal_intensity:.1f} dB")
...
class TaskValidator:
    ...
    def assess_training_adequacy(self,
                               task_id: str,
                               participant_id: str,
                               training_responses: List[SimulatedResponse]) -> TrainingAssessment:
        """
        ...
        stability_window = min(self.config['training_criteria']['stability_window'], len(accuracies))
        ...
        sufficient_learning = learning_curve_slope >= -criteria['learning_threshold']  # Not declining
        
        criterion_met = (sufficient_trials and 
                        sufficient_performance and 
                        sufficient_stability and 
                        sufficient_learning)
    ...
    def validate_task_design(self,
                           task_id: str,
                           stimuli: List[StimulusProperties],
                           task_duration: float,
                           response_window: float) -> TaskValidationResult:
        """
        ...
        if task_duration < requirements['minimum_task_duration']:
            issues.append(f"Task duration too short: {task_duration:.1f}s < {requirements['minimum_task_duration']:.1f}s")
        
        if task_duration > requirements['maximum_task_duration']:
            issues.append(f"Task duration too long: {task_duration:.1f}s > {requirements['maximum_task_duration']:.1f}s")
        ...
        for stimulus in stimuli:
            ...
            if not is_valid:
                stimulus_issues.extend([f"Stimulus {stimulus.stimulus_id}: {issue}" for issue in stim_issues])
        ...
        performance_validation = (requirements['minimum_task_duration'] <= task_duration <= 
                                requirements['maximum_task_duration'])
        
        # Overall validation score
        validation_components = [stimulus_validation, training_validation, performance_validation]
        validation_score = sum(validation_components) / len(validation_components)
        
        # Generate recommendations
        if not stimulus_validation:
            recommendations.append("Review and adjust stimulus parameters for supraliminal presentation")
        
        if not performance_validation:
            recommendations.append("Adjust task timing parameters to meet experimental requirements")
...
class WellTrainedTaskChecker:
    ...
    def assess_task_proficiency(self,
                              participant_id: str,
                              task_id: str,
                              session_responses: List[List[SimulatedResponse]]) -> Dict[str, Any]:
        """
        ...
        performance_consistency = 1.0 - np.std(session_performances) if len(session_performances) > 1 else 1.0
        ...
        fatigue_resistance = np.mean(fatigue_resistance_scores) if fatigue_resistance_scores else 0.5
        ...
        correlation_met = cross_session_correlation >= readiness_criteria['cross_session_correlation']
        ...
        is_ready = (sufficient_sessions and 
                   proficiency_level in ['proficient', 'expert'] and
                   stability_met and 
                   correlation_met and 
                   fatigue_met and 
                   consistency_met)
        ...
        if not sufficient_sessions:
            recommendations.append(f"Complete {criteria['minimum_sessions'] - sessions_completed} more training sessions")
...
class ConsciousnessMeasurementValidator:
    ...
    def assess_metacognitive_sensitivity(self,
                                       participant_id: str,
                                       primary_responses: List[bool],
                                       confidence_ratings: List[float],
                                       correct_answers: List[bool]) -> MetacognitiveAssessment:
        """
        ...
        if len(primary_responses) != len(confidence_ratings) or len(primary_responses) != len(correct_answers):
            raise ValueError("All input lists must have the same length")
        
        # Calculate Type 1 performance (primary task accuracy)
        type_1_accuracy = np.mean([resp == correct for resp, correct in zip(primary_responses, correct_answers)])
        
        # Calculate confidence-accuracy correlation
        accuracies = [1.0 if resp == correct else 0.0 for resp, correct in zip(primary_responses, correct_answers)]
        ...
        correct_trials = [i for i, (resp, correct) in enumerate(zip(primary_responses, correct_answers)) if resp == correct]
        incorrect_trials = [i for i, (resp, correct) in enumerate(zip(primary_responses, correct_answers)) if resp != correct]
        ...
        for i in range(n_bins):
            bin_mask = (confidence_ratings >= bin_edges[i]) & (confidence_ratings < bin_edges[i + 1])
            ...
            if np.any(bin_mask):
                ...
                calibration_scores.append(1.0 - calibration_error)  # Convert to score (higher = better)
        ...
        is_intact = (meta_d_prime >= thresholds['metacognitive_threshold'] and
                    conf_acc_correlation >= thresholds['confidence_accuracy_r'] and
                    calibration_score >= thresholds['calibration_threshold'])
    ...
    def validate_consciousness_measure(self,
                                     measure_type: str,
                                     values: List[float],
                                     reference_values: Optional[List[float]] = None) -> ConsciousnessMeasure:
        """
        ...
        if std_value > 0.05:  # Some minimum variability expected
            variability_validity = min(1.0, std_value / 0.3)  # Normalize to reasonable range
        else:
    ...
    def generate_consciousness_validation_report(self,
                                               ...
                                               correct_answers: List[bool]) -> ConsciousnessValidationReport:
        """
        ...
        """
        logger.info(f"Generating consciousness validation report for {participant_id}, session {session_id}")
        ...
        subjective_measure = self.validate_consciousness_measure('subjective_report', subjective_values)
        measures.append(subjective_measure)
        
        # Forced choice
        forced_choice_values = [1.0 if resp else 0.0 for resp in forced_choice_responses]
        forced_choice_measure = self.validate_consciousness_measure('forced_choice', forced_choice_values)
        ...
        overall_validity = (sufficient_measures and 
                          sufficient_convergence and 
                          sufficient_reliability and 
                          intact_metacognition)
        
        # Generate recommendations
        recommendations = []
        if not sufficient_measures:
            recommendations.append(f"Add {criteria['minimum_measures'] - len(measures)} more consciousness measures")
        ...
        if not sufficient_reliability:
            recommendations.append("Increase measurement reliability through more trials or better instructions")
        
        if not intact_metacognition:
            recommendations.append("Address metacognitive sensitivity issues - consider participant exclusion")
...
class ExperimentalIntegrityChecker:
    ...
    def check_response_system_integrity(self,
                                      participant_id: str,
                                      response_data: Dict[str, List[float]]) -> IntegrityCheckResult:
        """
            ...
            try:
                ...
                if 'response_times' in data and 'accuracies' in data:
                    ...
                    if validation.is_intact:
                        ...
                        issues.append(f"{system_type.value} system integrity compromised: {validation.notes}")
                        system_scores.append(0.5)  # Partial score for compromised system
                        recommendations.append(f"Investigate {system_type.value} response system issues")
    ...
    def check_consciousness_measurement_validity(self,
                                               participant_id: str,
                                               session_id: str,
                                               consciousness_data: Dict[str, List]) -> IntegrityCheckResult:
        """
        ...
        """
        logger.info(f"Checking consciousness measurement validity for {participant_id}, session {session_id}")
        ...
        try:
            ...
            validation_report = self.consciousness_validator.generate_consciousness_validation_report(
                participant_id, session_id, subjective_reports, forced_choice,
                confidence_ratings, wagering_responses, correct_answers
            )
            
            passed = validation_report.overall_validity
            score = (validation_report.convergent_validity + 
                    validation_report.measurement_reliability +
                    (1.0 if validation_report.metacognitive_assessment.is_intact else 0.0)) / 3.0
    ...
    def check_experimental_condition_compliance(self,
                                              condition: ExperimentalCondition,
                                              actual_parameters: Dict[str, Any]) -> IntegrityCheckResult:
        """
            ...
            if param_name in actual_parameters:
                ...
                if isinstance(expected_value, (int, float)):
                    ...
                    if abs(actual_value - expected_value) <= tolerance * abs(expected_value):
                        ...
                        issues.append(f"Parameter {param_name}: expected {expected_value}, got {actual_value}")
    ...
    def check_data_quality(self,
                          participant_id: str,
                          session_data: Dict[str, Any]) -> IntegrityCheckResult:
        """
        ...
        required_fields = ['response_times', 'accuracies', 'confidence_ratings', 'trial_timestamps']
        ...
        if 'response_times' in session_data and 'accuracies' in session_data:
            ...
            if rt_length == acc_length:
                ...
                issues.append(f"Data length mismatch: {rt_length} response times vs {acc_length} accuracies")
        ...
        if 'response_times' in session_data:
            ...
            if response_times:
                ...
                outliers = np.sum((rt_array < q1 - outlier_threshold) | (rt_array > q3 + outlier_threshold))
    ...
    def generate_integrity_report(self,
                                ...
                                participant_id: str,
                                session_id: str,
                                experimental_data: Dict[str, Any]) -> ExperimentalIntegrityReport:
        """
        ...
        """
        logger.info(f"Generating integrity report for experiment {experiment_id}, participant {participant_id}")
        ...
        if 'experimental_condition' in experimental_data and 'actual_parameters' in experimental_data:
        ...
        sufficient_score = overall_integrity_score >= self.config['integrity_thresholds']['minimum_overall_score']
    ...
    def validate_experiment_batch(self,
                                experiments: List[Dict[str, Any]]) -> Dict[str, ExperimentalIntegrityReport]:
        """
        ...
        for exp_data in experiments:
            try:
                ...
                logger.error(f"Error validating experiment {exp_data.get('experiment_id', 'unknown')}: {e}")
                # Create error report
