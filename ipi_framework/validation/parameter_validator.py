...
Provides comprehensive validation for all IPI Framework parameters including
...
class ParameterValidator:
    """
    Comprehensive parameter validator for IPI Framework.
    ...
    IPI_PARAM_RANGES = {
        'extero_precision': (0.01, 10.0, "Exteroceptive precision must be positive (typical: 0.5-5.0)"),
        'intero_precision': (0.01, 10.0, "Interoceptive precision must be positive (typical: 0.5-5.0)"),
        'extero_error': (-10.0, 10.0, "Exteroceptive error should be standardized z-score (typical: -3 to 3)"),
        'intero_error': (-10.0, 10.0, "Interoceptive error should be standardized z-score (typical: -3 to 3)"),
        'somatic_gain': (0.01, 5.0, "Somatic gain must be positive (typical: 0.5-2.0)"),
        'threshold': (0.1, 10.0, "Ignition threshold must be positive (typical: 2.0-5.0)"),
        'steepness': (0.1, 10.0, "Steepness parameter must be positive (typical: 1.0-3.0)"),
    }
    
    EXPERIMENTAL_PARAM_RANGES = {
        'n_trials': (1, 100000, "Number of trials must be positive (typical: 50-1000)"),
        'n_participants': (1, 10000, "Number of participants must be positive (typical: 20-200)"),
        'alpha_level': (0.001, 0.1, "Alpha level must be between 0.001 and 0.1 (typical: 0.05)"),
        'effect_size_threshold': (0.1, 2.0, "Effect size threshold (typical: 0.3-0.8)"),
        'power_threshold': (0.5, 0.99, "Statistical power threshold (typical: 0.8-0.95)"),
    }
    
    NEURAL_SIGNATURE_RANGES = {
        'p3b_threshold': (1.0, 20.0, "P3b amplitude threshold in μV (typical: 3.0-7.0)"),
    ...
    def validate_ipi_parameters(self, **params) -> ValidationResult:
        """
        Validate IPI equation parameters.
        
        Args:
            **params: IPI parameters to validate
        ...
        required_params = ['extero_precision', 'intero_precision', 'extero_error', 
                          'intero_error', 'somatic_gain', 'threshold', 'steepness']
        ...
        for param_name, value in params.items():
            if param_name not in self.IPI_PARAM_RANGES:
                warnings.append(f"Unknown parameter: {param_name}")
                continue
            
            min_val, max_val, description = self.IPI_PARAM_RANGES[param_name]
            ...
            if not (min_val <= value <= max_val):
                errors.append(f"{param_name}: Value {value} outside valid range [{min_val}, {max_val}]. {description}")
            
            # Warnings for unusual values
            if param_name in ['extero_precision', 'intero_precision']:
                if value < 0.5:
                    warnings.append(f"{param_name}: Very low precision ({value}) may indicate weak signal")
                elif value > 5.0:
                    warnings.append(f"{param_name}: Very high precision ({value}) is unusual")
            
            if param_name in ['extero_error', 'intero_error']:
                if abs(value) > 3.0:
                    warnings.append(f"{param_name}: Large error ({value}) is unusual for standardized values")
            
            if param_name == 'somatic_gain':
                if value < 0.8:
                    suggestions.append(f"Low somatic gain ({value}) reduces interoceptive influence")
                elif value > 2.0:
                    suggestions.append(f"High somatic gain ({value}) strongly amplifies interoceptive signals")
        ...
        if 'extero_precision' in params and 'intero_precision' in params:
            ...
            if ratio > 2.0:
                suggestions.append(f"Interoceptive precision is {ratio:.1f}x higher than exteroceptive - strong interoceptive bias")
            elif ratio < 0.5:
                suggestions.append(f"Exteroceptive precision is {1/ratio:.1f}x higher than interoceptive - strong exteroceptive bias")
    ...
    def validate_experimental_config(self, **config) -> ValidationResult:
        ...
        for param_name, value in config.items():
            ...
            if param_name == 'n_trials':
                if value < 50:
                    warnings.append(f"Low trial count ({value}) may have insufficient statistical power")
                elif value > 5000:
                    warnings.append(f"Very high trial count ({value}) may be computationally expensive")
        ...
        if 'n_trials' in config and 'n_participants' in config:
            ...
            if total_observations < 1000:
                suggestions.append(f"Total observations ({total_observations}) may be low for robust statistics")
            elif total_observations > 100000:
                suggestions.append(f"Total observations ({total_observations}) is very large - consider reducing for efficiency")
    ...
    def validate_neural_thresholds(self, **thresholds) -> ValidationResult:
        ...
        for param_name, value in thresholds.items():
            ...
            if param_name == 'p3b_threshold':
                if value < 3.0:
                    suggestions.append(f"Low P3b threshold ({value} μV) may increase false positives")
                elif value > 7.0:
                    suggestions.append(f"High P3b threshold ({value} μV) may miss genuine ignition events")
            ...
            if param_name == 'bold_z_threshold':
                if value < 2.3:
                    warnings.append(f"BOLD threshold ({value}) below conventional significance (Z=2.3, p<0.05)")
                elif value > 3.5:
    ...
    def validate_pharmacological_condition(self, drug_type: str, dosage: float, 
                                         administration_time: float) -> ValidationResult:
        """
        ...
        if drug_type.lower() in dosage_ranges:
            ...
            if dosage < 0:
                ...
                warnings.append(f"{drug_type} dosage {dosage} {unit} outside typical range [{min_dose}-{max_dose}] {unit}")
        ...
        if administration_time < 0:
            ...
            warnings.append(f"Long administration time ({administration_time} min) - drug effects may have peaked")
        
        # Drug-specific suggestions
        if drug_type.lower() == 'propranolol':
            if administration_time < 30:
                suggestions.append("Propranolol typically requires 30-60 minutes to reach peak effect")
    ...
    def validate_all(self, ipi_params: Optional[Dict] = None,
                    experimental_config: Optional[Dict] = None,
                    neural_thresholds: Optional[Dict] = None) -> ValidationResult:
        """
        Validate all parameter types at once.
        
        Args:
            ipi_params: IPI equation parameters
        ...
        if ipi_params:
            result = self.validate_ipi_parameters(**ipi_params)
    ...
    def get_parameter_info(self, param_name: str) -> str:
        ...
        for param_dict, category in [
            (self.IPI_PARAM_RANGES, "IPI Equation Parameter"),
