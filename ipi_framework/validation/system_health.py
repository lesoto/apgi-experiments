...
Provides comprehensive system health checks and diagnostics for the IPI Framework.
"""
...
class HealthCheckResult:
    ...
    def get_report(self) -> str:
        ...
        lines = []
        lines.append("=" * 60)
        lines.append("IPI FRAMEWORK SYSTEM HEALTH CHECK")
        lines.append("=" * 60)
        lines.append(f"Timestamp: {self.timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append(f"Overall Status: {self.overall_status.upper()}")
        lines.append(f"Checks: {self.checks_passed} passed, {self.checks_warning} warnings, {self.checks_failed} failed")
        lines.append("")
...
class SystemHealthChecker:
    """
    Comprehensive system health checker for IPI Framework.
    ...
    def _check_python_environment(self) -> tuple:
        ...
        if python_version < (3, 8):
            ...
            warnings.append(f"Python {python_version.major}.{python_version.minor} is supported but 3.9+ recommended")
    ...
    def _check_dependencies(self) -> tuple:
        ...
        for package, min_version in optional_packages.items():
            try:
                ...
                warnings.append(f"Optional package '{package}' not found (some features may be unavailable)")
    ...
    def _check_data_storage(self) -> tuple:
        ...
        try:
            import shutil
            total, used, free = shutil.disk_usage(results_dir if os.path.exists(results_dir) else ".")
            free_gb = free / (1024**3)
            if free_gb < 1.0:
    ...
    def _check_core_components(self) -> tuple:
        """Check core IPI Framework components"""
        issues = []
        warnings = []
        
        # Check core modules
        core_modules = [
            'ipi_framework.core.equation',
            'ipi_framework.simulators.p3b_simulator',
            'ipi_framework.simulators.gamma_simulator',
            'ipi_framework.falsification.primary_falsification_test',
        ]
        ...
        try:
            ...
            if not np.isfinite(test_result):
                issues.append("IPI equation produces invalid results")
        except Exception as e:
            issues.append(f"IPI equation test failed: {str(e)}")
    ...
    def _generate_recommendations(self, issues: List[str], warnings: List[str]) -> List[str]:
        ...
        if any("module" in issue.lower() for issue in issues):
            recommendations.append("Reinstall IPI Framework or check installation integrity")
