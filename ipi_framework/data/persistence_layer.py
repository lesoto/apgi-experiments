"""
Persistence layer for the IPI Framework data management system.
...
class PersistenceLayer:
    ...
    def _init_hdf5(self):
        ...
        if not self.hdf5_path.exists():
            with h5py.File(self.hdf5_path, 'w') as f:
                ...
                f.attrs['description'] = 'IPI Framework Experimental Data Storage'
    ...
    def load_dataset(self, experiment_id: str, version: Optional[str] = None) -> ExperimentalDataset:
        """
    ...
    def _load_backup_info(self, experiment_id: str) -> List[BackupInfo]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                "SELECT * FROM backups WHERE experiment_id = ?",
                (experiment_id,)
            )
    ...
    def create_backup(self, experiment_id: str, backup_type: str = "full") -> BackupInfo:
        ...
        try:
            ...
            backup_dir = self.backup_path / f"{experiment_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
