"""
Comprehensive Reporting Dashboard for IPI Framework
    ...
    def _update_experiment_statistics(self, experiment_id: str):
        ...
        if elapsed > 0 and exp['current_trial'] > 0:
            ...
            if exp['statistics']['trials_per_second'] > 0 and exp['total_trials'] > 0:
                ...
                exp['statistics']['estimated_completion'] = now + timedelta(seconds=remaining_seconds)
...
class ExperimentComparator:
    ...
    def _generate_comparison_summary(self, experiments: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """Generate high-level comparison summary"""
        
        total_experiments = len(experiments)
        completed_experiments = sum(1 for exp in experiments.values() if exp['status'] == 'completed')
        
        all_results = []
        for exp in experiments.values():
            all_results.extend(exp.get('results', []))
        
        falsification_rate = sum(1 for r in all_results if r.is_falsified) / len(all_results) if all_results else 0
    ...
    def _compare_statistics(self, experiments: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if stats_by_experiment:
            ...
            comparison_stats = {
                ...
                'most_powerful_experiment': max(stats_by_experiment.keys(), 
                                              key=lambda k: stats_by_experiment[k]['mean_power']),
                'highest_effect_size_experiment': max(stats_by_experiment.keys(),
        ...
        return {
            'by_experiment': stats_by_experiment,
    ...
    def _analyze_parameters(self, experiments: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for exp_data in experiments.values():
            ...
            for trial in trials:
                if hasattr(trial, 'ipi_parameters'):
                    params = trial.ipi_parameters
        ...
        for exp_id, exp_data in experiments.items():
            ...
            if exp_data['status'] == 'completed':
                ...
                if start_time and last_update:
                    ...
                    if runtime > 0:
                        total_trials = exp_data.get('total_trials', exp_data.get('current_trial', 0))
    ...
    def _generate_recommendations(self, experiments: Dict[str, Dict[str, Any]]) -> List[str]:
        ...
        if all_results:
            ...
            if falsification_rate > 0.5:
                recommendations.append(
                    "High falsification rate detected. Consider revising IPI Framework parameters or methodology."
                )
        ...
        completed_experiments = [exp for exp in experiments.values() if exp['status'] == 'completed']
...
class DashboardServer:
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        if len(self.dashboard_data['experiments']) > 1:
            try:
                comparison = self.comparator.compare_experiments(self.dashboard_data['experiments'])
