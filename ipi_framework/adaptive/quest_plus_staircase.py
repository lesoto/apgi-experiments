...
class QuestPlusStaircase:
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized QUEST+ staircase for participant {participant_id}, task {task_id}")
    ...
    def _initialize_parameter_spaces(self):
        ...
        logger.debug(f"Parameter spaces initialized: {len(self.stimulus_space)} stimuli, "
    ...
    def _expected_entropy(self, intensity: float) -> float:
        ...
        for i, threshold in enumerate(self.threshold_space):
            for j, slope in enumerate(self.slope_space):
                ...
                if prior_prob > 1e-10:  # Skip negligible probabilities
                    ...
                    # Response: detected
                    likelihood_detected = self._likelihood(intensity, True, threshold, slope)
                    posterior_detected = prior_prob * likelihood_detected
                    
                    # Response: not detected  
                    likelihood_not_detected = self._likelihood(intensity, False, threshold, slope)
                    ...
                    if posterior_not_detected > 1e-10:
                        entropy_not_detected += posterior_not_detected * np.log(posterior_not_detected)
        ...
        # Expected entropy
        expected_entropy = (p_detected_total * entropy_detected + 
    ...
    def update(self, intensity: float, response: bool) -> None:
        ...
        logger.debug(f"Updated staircase: trial {self.state.trial_number}, "
    ...
    def _update_threshold_estimate(self) -> None:
        ...
        threshold_variance = np.sum(threshold_marginal * 
    ...
    def _check_reversals(self, response: bool) -> None:
        ...
        if (current_direction is not None and 
            self.state.last_direction is not None and
            current_direction != self.state.last_direction):
            self.state.reversals += 1
            logger.debug(f"Reversal {self.state.reversals} detected at trial {self.state.trial_number}")
    ...
    def _check_convergence(self) -> None:
        ...
        if (self.state.trial_number < self.parameters.min_trials or
            self.state.reversals < self.parameters.min_reversals):
            return
        ...
        if len(self.state.intensities) >= 10:
            ...
            for i in range(len(self.state.intensities) - 5, len(self.state.intensities)):
                if i >= 5:  # Need some history for estimate
                    ...
                    if recent_responses and recent_intensities:
                        ...
                        hits = [intensity for intensity, response in 
                               zip(recent_intensities, recent_responses) if response]
                        misses = [intensity for intensity, response in 
                                 zip(recent_intensities, recent_responses) if not response]
            ...
            if len(recent_estimates) >= 3:
                ...
                if estimate_std < self.parameters.convergence_criterion:
                    ...
                    logger.info(f"Staircase converged at trial {self.state.trial_number}, "
                              f"threshold estimate: {self.state.threshold_estimate:.4f}")
        ...
        if self.state.trial_number >= self.parameters.max_trials:
            self.state.converged = True
            self.state.convergence_trial = self.state.trial_number
            logger.info(f"Staircase forced convergence at maximum trials ({self.parameters.max_trials})")
    ...
    def get_psychometric_curve(self, intensities: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def should_continue(self) -> bool:
        ...
        return (not self.state.converged and 
                self.state.trial_number < self.parameters.max_trials)
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'hit_rate': hit_rate,
            'intensity_range': (min(self.state.intensities), max(self.state.intensities)),
