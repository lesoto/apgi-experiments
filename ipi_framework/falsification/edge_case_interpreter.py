...
Implements edge case classification and interpretation for IPI Framework boundaries.
...
class EdgeCaseInterpreter:
    """
    Classifier and interpreter for edge cases in IPI Framework testing.
    ...
    def classify_edge_case(self, neural_signatures: CombinedSignature,
                          consciousness_assessment: ConsciousnessAssessment,
                          analysis_id: Optional[str] = None) -> EdgeCaseAnalysisResult:
        """
        ...
        primary_case_type = max(classification_scores.keys(), 
        ...
        primary_classification = self._create_classification(
            primary_case_type, primary_score, neural_signatures, consciousness_assessment
        )
        
        # Find alternative classifications (scores > threshold)
        alternative_classifications = []
        for case_type, score in classification_scores.items():
            if (case_type != primary_case_type and 
                score > self.classification_confidence_threshold):
        ...
        further_testing_needed = (primary_classification.confidence < 0.8 or 
                                len(alternative_classifications) > 0)
    ...
    def _calculate_classification_score(self, neural_signatures: CombinedSignature,
                                      consciousness_assessment: ConsciousnessAssessment,
                                      profile: EdgeCaseProfile) -> float:
    ...
    def _create_classification(self, case_type: EdgeCaseType, confidence: float,
                             neural_signatures: CombinedSignature,
                             consciousness_assessment: ConsciousnessAssessment) -> EdgeCaseClassification:
        ...
        # Check P3b evidence
        if profile.expected_p3b_range[0] <= neural_signatures.p3b_amplitude <= profile.expected_p3b_range[1]:
            supporting_evidence.append(f"P3b amplitude ({neural_signatures.p3b_amplitude:.2f} μV) within expected range")
        else:
            contradicting_evidence.append(f"P3b amplitude ({neural_signatures.p3b_amplitude:.2f} μV) outside expected range")
        
        # Check gamma evidence
        if profile.expected_gamma_range[0] <= neural_signatures.gamma_plv <= profile.expected_gamma_range[1]:
            supporting_evidence.append(f"Gamma PLV ({neural_signatures.gamma_plv:.3f}) within expected range")
        else:
            contradicting_evidence.append(f"Gamma PLV ({neural_signatures.gamma_plv:.3f}) outside expected range")
        
        # Check PCI evidence
        if profile.expected_pci_range[0] <= neural_signatures.pci_value <= profile.expected_pci_range[1]:
            supporting_evidence.append(f"PCI ({neural_signatures.pci_value:.3f}) within expected range")
        else:
            contradicting_evidence.append(f"PCI ({neural_signatures.pci_value:.3f}) outside expected range")
        ...
        if consciousness_assessment.subjective_report == expected_report:
            supporting_evidence.append(f"Subjective report matches expectation ({consciousness_assessment.subjective_report})")
        else:
            contradicting_evidence.append(f"Subjective report contradicts expectation ({consciousness_assessment.subjective_report})")
        ...
        consciousness_consistency = self._calculate_consciousness_consistency(consciousness_assessment, profile)
        
        # Determine framework boundary
        boundary_confidence = min(confidence, 0.9)  # Cap at 0.9 for boundary decisions
        
        # Generate interpretation
        interpretation = self._generate_interpretation(case_type, profile, supporting_evidence, contradicting_evidence)
    ...
    def _calculate_signature_consistency(self, neural_signatures: CombinedSignature,
                                       profile: EdgeCaseProfile) -> float:
        """Calculate consistency of neural signatures with profile expectations"""
        scores = []
        
        # P3b consistency
        p3b_score = self._calculate_range_score(neural_signatures.p3b_amplitude, profile.expected_p3b_range)
        scores.append(p3b_score)
        
        # Gamma consistency
        gamma_score = self._calculate_range_score(neural_signatures.gamma_plv, profile.expected_gamma_range)
        scores.append(gamma_score)
        
        # PCI consistency
        pci_score = self._calculate_range_score(neural_signatures.pci_value, profile.expected_pci_range)
        scores.append(pci_score)
    ...
    def _calculate_consciousness_consistency(self, consciousness_assessment: ConsciousnessAssessment,
                                          profile: EdgeCaseProfile) -> float:
    ...
    def _generate_interpretation(self, case_type: EdgeCaseType, profile: EdgeCaseProfile,
                               supporting_evidence: List[str], contradicting_evidence: List[str]) -> str:
        ...
        interpretation_parts.append(f"Framework Boundary: {profile.framework_boundary.value.replace('_', ' ').title()}")
        
        if supporting_evidence:
            interpretation_parts.append(f"Supporting Evidence: {'; '.join(supporting_evidence)}")
        
        if contradicting_evidence:
            interpretation_parts.append(f"Contradicting Evidence: {'; '.join(contradicting_evidence)}")
        
        interpretation_parts.append(f"Clinical Notes: {profile.interpretation_notes}")
        
        return " | ".join(interpretation_parts)
    
    def _generate_recommendations(self, case_type: EdgeCaseType, profile: EdgeCaseProfile,
                                contradicting_evidence: List[str]) -> List[str]:
        """Generate recommendations based on classification"""
        recommendations = []
        
        if contradicting_evidence:
            recommendations.append("Consider alternative classifications due to contradicting evidence")
        ...
        elif case_type in [EdgeCaseType.VEGETATIVE_STATE, EdgeCaseType.MINIMALLY_CONSCIOUS_STATE]:
            recommendations.extend([
    ...
    def _generate_framework_implications(self, classification: EdgeCaseClassification,
                                       neural_signatures: CombinedSignature,
                                       consciousness_assessment: ConsciousnessAssessment) -> str:
        ...
        if classification.framework_boundary == FrameworkBoundary.WITHIN_FRAMEWORK:
            implications.append("This case falls within expected IPI Framework boundaries.")
            ...
            if classification.case_type == EdgeCaseType.NORMAL_CONTROL:
                ...
                implications.append("Preserved consciousness with motor impairment supports framework.")
            ...
            implications.append("This case represents a boundary condition for the IPI Framework.")
            ...
            if classification.case_type == EdgeCaseType.ANESTHESIA_AWARENESS:
                implications.append("Altered consciousness under anesthesia challenges standard ignition patterns.")
                ...
                implications.append("Fluctuating consciousness suggests variable ignition thresholds.")
                
        elif classification.framework_boundary == FrameworkBoundary.OUTSIDE_FRAMEWORK:
            implications.append("This case falls outside current IPI Framework scope.")
            ...
            implications.append("This case potentially violates core IPI Framework assumptions.")
            implications.append("Significant framework revision may be required.")
        
        return " ".join(implications)
    
    def _assess_falsification_relevance(self, classification: EdgeCaseClassification,
                                      neural_signatures: CombinedSignature,
                                      consciousness_assessment: ConsciousnessAssessment) -> str:
        ...
        has_full_signatures = (neural_signatures.p3b_amplitude > 5.0 and
                             neural_signatures.gamma_plv > 0.3 and
                             neural_signatures.pci_value > 0.4 and
                             any(z > 3.1 for z in neural_signatures.bold_activations.values()))
        
        has_consciousness = consciousness_assessment.subjective_report
        
        if has_full_signatures and not has_consciousness:
            relevance_parts.append("CRITICAL: Full ignition signatures without consciousness - potential primary falsification")
        elif has_consciousness and not has_full_signatures:
            relevance_parts.append("IMPORTANT: Consciousness without full ignition - potential secondary falsification")
        else:
        ...
        if classification.case_type == EdgeCaseType.ANESTHESIA_AWARENESS:
            ...
            relevance_parts.append("Blindsight demonstrates unconscious processing without ignition")
        elif classification.case_type == EdgeCaseType.DREAMS:
            relevance_parts.append("Dream consciousness shows altered ignition patterns")
        
        return " | ".join(relevance_parts)
    
    def _generate_test_recommendations(self, primary_classification: EdgeCaseClassification,
                                     alternative_classifications: List[EdgeCaseClassification]) -> List[str]:
        ...
        if primary_classification.confidence < 0.8:
            recommendations.append("Low classification confidence - conduct additional measurements")
    ...
    def validate_framework_boundaries(self, analysis_results: List[EdgeCaseAnalysisResult]) -> Dict[str, Any]:
        """
        ...
        for result in analysis_results:
            ...
            if result.primary_classification.framework_boundary == FrameworkBoundary.FRAMEWORK_VIOLATION:
                violation_cases.append(result)
            elif result.primary_classification.framework_boundary == FrameworkBoundary.BOUNDARY_CASE:
    ...
    def _generate_boundary_validation_recommendations(self, violation_rate: float,
                                                    boundary_rate: float,
        ...
        if violation_rate > 0.1:  # More than 10% violations
            recommendations.append("High violation rate detected - framework may need significant revision")
            recommendations.append("Conduct systematic review of violation cases")
            recommendations.append("Consider expanding framework scope or modifying core assumptions")
        
        if boundary_rate > 0.3:  # More than 30% boundary cases
            recommendations.append("High boundary case rate - framework boundaries may need clarification")
