...
class SomaBiasTest:
    ...
    def run_soma_bias_test(self, 
                          ...
                          test_id: Optional[str] = None) -> SomaBiasTestResult:
        """
    ...
    def _run_single_trial(self, trial_id: str, participant_id: str) -> SomaBiasTrialResult:
        ...
        extero_precision, intero_precision, match_quality = self._generate_matched_precision()
        ...
        beta_value = self._calculate_beta_coefficient(
            extero_error, intero_error, extero_precision, intero_precision,
            extero_ignition_prob, intero_ignition_prob
        )
    ...
    def _calculate_ignition_probability(self, error: float, precision: float, 
                                      domain: str) -> float:
    ...
    def _calculate_beta_coefficient(self, extero_error: float, intero_error: float,
                                  extero_precision: float, intero_precision: float,
                                  extero_ignition: float, intero_ignition: float) -> float:
        """
        Calculate bias coefficient β.
    ...
    def _calculate_confidence_level(self, beta_value: float, match_quality: float,
                                  ignition_difference: float) -> float:
    ...
    def _analyze_results(self, test_id: str, 
                        ...
                        n_participants: int) -> SomaBiasTestResult:
        ...
        intero_bias_participants = sum(1 for bias in participant_bias_directions.values() 
                                     if bias == "intero")
        extero_bias_participants = sum(1 for bias in participant_bias_directions.values() 
                                     if bias == "extero")
        no_bias_participants = sum(1 for bias in participant_bias_directions.values() 
                                 if bias == "none")
        ...
        is_falsified = (meets_sample_size_requirement and 
                       no_bias_proportion > self.falsification_threshold and
                       population_p_value < 0.05)
        
        # Generate interpretation
        interpretation = self._generate_interpretation(
            mean_beta, no_bias_proportion, is_falsified, 
            intero_bias_participants, extero_bias_participants, no_bias_participants
        )
    ...
    def _generate_interpretation(self, mean_beta: float, no_bias_proportion: float,
                               is_falsified: bool, intero_bias_count: int,
                               extero_bias_count: int, no_bias_count: int) -> str:
        """Generate interpretation of soma-bias test results"""
        total_participants = intero_bias_count + extero_bias_count + no_bias_count
        
        if is_falsified:
            return (f"FRAMEWORK FALSIFIED: {no_bias_count}/{total_participants} participants "
                   f"({no_bias_proportion:.1%}) showed no soma-bias (β ≈ 1.0). "
                   f"Population mean β = {mean_beta:.3f}. This contradicts the IPI Framework's "
                   f"prediction that interoceptive signals should receive preferential weighting.")
        
        elif no_bias_proportion > 0.3:
            return (f"PARTIAL SOMA-BIAS ABSENCE: {no_bias_count}/{total_participants} participants "
            ...
            return (f"STRONG INTEROCEPTIVE BIAS: Population mean β = {mean_beta:.3f} with "
                   f"{intero_bias_count}/{total_participants} participants showing interoceptive bias. "
                   f"Results strongly support the IPI Framework's prediction of soma-bias.")
        
        elif mean_beta < 0.8:
            return (f"UNEXPECTED EXTEROCEPTIVE BIAS: Population mean β = {mean_beta:.3f} with "
            ...
            return (f"MODERATE SOMA-BIAS: Population mean β = {mean_beta:.3f} with mixed bias patterns. "
    ...
    def validate_precision_matching(self, extero_precision: float, 
                                  intero_precision: float,
        ...
        relative_difference = abs(extero_precision - intero_precision) / max(extero_precision, intero_precision)
    ...
    def simulate_individual_differences(self, n_participants: int) -> Dict[str, Dict[str, float]]:
        """
