...
manipulations that should modulate it according to the IPI Framework.
"""
...
class ThresholdInsensitivityTest:
    ...
    challenges that should modulate it according to the IPI Framework:
    ...
    def run_threshold_insensitivity_test(self, 
                                       ...
                                       test_id: Optional[str] = None) -> ThresholdInsensitivityTestResult:
        """
    ...
    def _run_single_trial(self, trial_id: str, participant_id: str, 
        ...
        physiological_controls_valid = self._validate_physiological_controls(drug_condition)
        ...
        is_falsifying = self._determine_falsification(
            drug_type, threshold_change, physiological_controls_valid, drug_effect_confirmed
        )
    ...
    def _simulate_control_measures(self, drug_type: DrugType, dosage: float) -> Dict[str, float]:
        ...
        if drug_type == DrugType.PROPRANOLOL:
            ...
            controls['blood_pressure_change'] = -np.random.uniform(5, 15)  # Decreased BP
            ...
            controls['attention_score'] = np.random.uniform(1.05, 1.2)  # Enhanced attention
            controls['pupil_dilation'] = np.random.uniform(1.1, 1.25)  # Increased dilation
        
        elif drug_type == DrugType.SSRI:
            # Serotonergic effects
            controls['mood_rating'] = np.random.uniform(1.05, 1.15)  # Slight mood improvement
            ...
            controls['attention_score'] = np.random.uniform(1.15, 1.35)  # Enhanced attention
    ...
    def _simulate_post_drug_threshold(self, baseline_threshold: float, 
                                    drug_condition: PharmacologicalCondition) -> float:
        """Simulate post-drug threshold with potential insensitivity"""
        expected_change = drug_condition.expected_threshold_change
        
        # Simulate varying degrees of sensitivity/insensitivity
        sensitivity_factor = np.random.uniform(0.0, 1.2)  # 0 = completely insensitive, 1+ = normal/hypersensitive
    ...
    def _validate_physiological_controls(self, drug_condition: PharmacologicalCondition) -> bool:
    ...
    def _confirm_drug_effect(self, drug_condition: PharmacologicalCondition) -> bool:
        ...
        effectiveness_probability = min(0.95, dosage / 100.0)  # Higher dose = more likely effective
        return np.random.random() < effectiveness_probability
    
    def _calculate_sensitivity_confidence(self, threshold_change: float, 
                                        expected_change: float) -> float:
        ...
        magnitude_ratio = abs(threshold_change) / abs(expected_change) if expected_change != 0 else 0
        
        if direction_match:
            confidence = 0.5 + 0.3 * min(1.0, magnitude_ratio)
        else:
            confidence = 0.5 - 0.2 * min(1.0, abs(threshold_change) / abs(expected_change))
        
        return np.clip(confidence, 0.0, 1.0)
    
    def _determine_falsification(self, drug_type: DrugType, threshold_change: float,
                               physiological_controls_valid: bool, 
                               drug_effect_confirmed: bool) -> bool:
        """
    ...
    def _calculate_confidence_level(self, change_difference: float,
                                  physiological_controls_valid: bool,
                                  drug_effect_confirmed: bool) -> float:
    ...
    def _analyze_results(self, test_id: str, 
                        trial_results: List[ThresholdInsensitivityTrialResult],
                        n_trials_per_condition: int, n_participants: int,
                        drug_conditions: List[DrugType]) -> ThresholdInsensitivityTestResult:
        ...
        for drug_type in drug_conditions:
            drug_trials = [r for r in trial_results if r.drug_condition.drug_type == drug_type]
            ...
            drug_sensitivity_results[drug_type.value] = {
                ...
                'insensitivity_rate': len(drug_insensitive) / len(drug_trials) if drug_trials else 0,
                'mean_threshold_change': np.mean([r.threshold_change for r in drug_trials]),
                'expected_change': self.expected_changes[drug_type],
                'mean_confidence': np.mean([r.confidence_level for r in drug_trials])
            }
        
        # Perform statistical analysis
        p_value = self._calculate_p_value(insensitivity_rate, total_insensitive, len(trial_results))
        ...
        is_falsified = insensitivity_rate > self.insensitivity_threshold and p_value < 0.05
        
        # Generate interpretation
        interpretation = self._generate_interpretation(
            insensitivity_rate, is_falsified, total_insensitive, drug_sensitivity_results
        )
    ...
    def _calculate_p_value(self, insensitivity_rate: float, 
                          n_insensitive: int, n_total: int) -> float:
    ...
    def _calculate_effect_size(self, insensitive_trials: List, all_trials: List) -> float:
        ...
        sensitive_changes = [abs(t.threshold_change) for t in all_trials 
                           if not t.is_falsifying]
        
        if len(sensitive_changes) == 0:
            return 1.0
        
        mean_diff = np.mean(sensitive_changes) - np.mean(insensitive_changes)
        pooled_std = np.sqrt((np.var(insensitive_changes) + np.var(sensitive_changes)) / 2)
    ...
    def _generate_interpretation(self, insensitivity_rate: float, 
                               is_falsified: bool, total_insensitive: int,
                               drug_results: Dict[str, Dict[str, Any]]) -> str:
        """Generate interpretation of threshold insensitivity results"""
        if is_falsified:
            # Identify which drugs showed insensitivity
            insensitive_drugs = [drug for drug, results in drug_results.items() 
                               if results['insensitivity_rate'] > 0.5 and drug != 'placebo']
            
            return (f"FRAMEWORK FALSIFIED: {total_insensitive} trials ({insensitivity_rate:.1%}) "
                   f"showed threshold insensitivity to neuromodulatory challenges. "
                   f"Drugs showing insensitivity: {', '.join(insensitive_drugs)}. "
                   f"This contradicts the IPI Framework's prediction that θₜ should be "
                   f"modulated by neuromodulatory systems.")
        
        elif insensitivity_rate > 0.3:
            return (f"PARTIAL INSENSITIVITY: {total_insensitive} trials ({insensitivity_rate:.1%}) "
            ...
            return (f"THRESHOLD SENSITIVITY CONFIRMED: Only {total_insensitive} trials "
                   f"({insensitivity_rate:.1%}) showed insensitivity to neuromodulatory "
                   f"challenges. Results support the IPI Framework's prediction that "
