...
Implements the primary falsification test for the IPI Framework, testing whether
...
class PrimaryFalsificationTest:
    """
    Primary falsification test controller for the IPI Framework.
    ...
    def run_falsification_test(self, 
                             ...
                             test_id: Optional[str] = None) -> FalsificationTestResult:
        """
    ...
    def _validate_signatures(self, signatures: Dict) -> Dict:
        ...
        gamma_met = (signatures['gamma_plv'] > self.gamma_plv_threshold and 
                    signatures['gamma_duration'] > self.gamma_duration_threshold)
        
        # Check BOLD activation in key regions
        key_regions = ['dlpfc_left', 'dlpfc_right', 'ips_left', 'ips_right', 
                      'anterior_insula']
        bold_met = all(signatures['bold_activations'].get(region, 0) > self.bold_z_threshold 
    ...
    def _assess_consciousness(self) -> Dict:
        ...
        subjective_report = np.random.choice([True, False], p=[0.3, 0.7])  # Bias toward absent
        
        # Simulate forced-choice accuracy (should be at chance level for absent consciousness)
        if subjective_report:
            forced_choice_accuracy = np.random.normal(0.75, 0.1)  # Above chance if conscious
        else:
            forced_choice_accuracy = np.random.normal(0.5, 0.05)  # At chance if unconscious
    ...
    def _determine_falsification(self, signature_validation: Dict, 
                               consciousness_assessment: Dict,
                               ai_acc_validation: Dict, 
                               controls_valid: bool) -> bool:
        """
    ...
    def _calculate_confidence_level(self, signature_validation: Dict, 
                                  consciousness_assessment: Dict) -> float:
    ...
    def _analyze_results(self, test_id: str, trial_results: List[FalsificationTrialResult],
                        n_trials: int, n_participants: int) -> FalsificationTestResult:
    ...
    def _calculate_effect_size(self, falsifying_trials: List, all_trials: List) -> float:
        ...
        non_falsifying_confidence = [t.confidence_level for t in all_trials 
        ...
        pooled_std = np.sqrt((np.var(falsifying_confidence) + np.var(non_falsifying_confidence)) / 2)
    ...
    def _generate_interpretation(self, falsification_rate: float, 
                               is_falsified: bool, total_falsifying: int) -> str:
        """Generate interpretation of results"""
        if is_falsified:
            return (f"FRAMEWORK FALSIFIED: {total_falsifying} trials showed full ignition "
                   f"signatures without consciousness ({falsification_rate:.1%} rate). "
                   f"This provides strong evidence against the IPI Framework's core prediction.")
        elif falsification_rate > 0:
            return (f"PARTIAL FALSIFICATION: {total_falsifying} trials showed potential "
            ...
            return ("NO FALSIFICATION: No trials showed full ignition signatures without "
                   "consciousness. Results support the IPI Framework's predictions.")