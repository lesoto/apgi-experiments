...
class ParameterEstimationUserManual:
    """
    Generates comprehensive user manual for IPI Framework parameter estimation.
    ...
    def __init__(self, output_dir: Optional[Path] = None):
        ...
        self.output_dir = output_dir or Path.home() / '.ipi_framework' / 'documentation'
    ...
    def generate_complete_manual(self) -> Path:
        ...
        manual_path = self.output_dir / 'IPI_Parameter_Estimation_User_Manual.md'
    ...
    def _generate_title_page(self) -> str:
        """Generate title page."""
        return f"""# IPI Framework Parameter Estimation System
## User Manual

**Version:** 1.0  
**Date:** {datetime.now().strftime('%B %d, %Y')}  
**Authors:** IPI Framework Development Team
...
The IPI Framework Parameter Estimation System is a comprehensive neuroscience research platform designed to rapidly quantify three key computational parameters within a standardized 45-60 minute protocol:
...
The IPI Framework consists of several integrated components:
...
2. **Install IPI Framework**
   ```bash
   # Clone repository
   git clone https://github.com/ipi-framework/parameter-estimation.git
...
3. **Verify Installation**
   ...
   python -m ipi_framework.deployment.deployment_validator
   ```

4. **Configure Hardware**
   ```python
   from ipi_framework.deployment import HardwareConfigurationManager
...
```python
from ipi_framework.tasks import DetectionTask
...
```python
from ipi_framework.tasks import HeartbeatDetectionTask
...
```python
from ipi_framework.tasks import DualModalityOddballTask
...
For technical support, contact: support@ipi-framework.org  
For updates and documentation: https://ipi-framework.org/docs
...
"""
    ...
    def generate_quick_start_guide(self) -> Path:
        ...
        with open(guide_path, 'w', encoding='utf-8') as f:
            f.write("""# IPI Framework Quick Start Guide

## 5-Minute Setup

### 1. Install Software
```bash
pip install ipi-framework
```
...
### 3. Run Validation
```python
from ipi_framework.deployment import DeploymentValidator
...
```python
from ipi_framework.gui import ParameterEstimationGUI
...
- Full Manual: `IPI_Parameter_Estimation_User_Manual.md`
- Troubleshooting: `Troubleshooting_Guide.md`
- Support: support@ipi-framework.org
""")
