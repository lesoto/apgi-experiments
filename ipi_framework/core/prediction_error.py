"""
Prediction error processing module for the IPI Framework.

This module implements prediction error processing and z-score standardization
for both exteroceptive and interoceptive prediction errors used in the IPI equation.
"""
...
class PredictionErrorProcessor:
    ...
    prediction errors before they are used in the IPI equation.
    """
    ...
    def process_exteroceptive_error(self,
                                   error: Union[float, np.ndarray],
                                   reference_mean: Optional[float] = None,
                                   reference_std: Optional[float] = None) -> Union[float, np.ndarray]:
        """
    ...
    def process_interoceptive_error(self,
                                   error: Union[float, np.ndarray],
                                   reference_mean: Optional[float] = None,
                                   reference_std: Optional[float] = None) -> Union[float, np.ndarray]:
        """
    ...
    def validate_error_pair(self,
                           extero_error: float,
                           intero_error: float) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if self.standardize:
                if abs(extero_error) > 5:
                    return False, f"Exteroceptive error {extero_error} is an extreme outlier"
                
                if abs(intero_error) > 5:
                    return False, f"Interoceptive error {intero_error} is an extreme outlier"
