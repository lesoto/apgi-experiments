"""
Joint parameter estimation pipeline for IPI framework.
...
class ConvergenceDiagnostics:
    ...
    def __repr__(self) -> str:
        ...
        return (f"ConvergenceDiagnostics(status={status}, "
...
class ParameterExtractor:
    ...
    def extract_all_parameters(
        ...
        for subj_idx in range(n_subjects):
            ...
            estimates.append(ParameterEstimates(
    ...
    def extract_population_parameters(
        ...
        for param_name in ['mu_theta0', 'sigma_theta0', 
                          'mu_pi_i', 'sigma_pi_i',
                          'mu_beta', 'sigma_beta']:
            samples = fit_result.extract(param_name)[param_name]
            population_params[param_name] = \
                ParameterExtractor.extract_parameter_distribution(
                    samples, param_name
                )
...
class ConvergenceDiagnosticsCalculator:
    ...
    def assess_convergence(
        ...
        param_names = ['theta0', 'pi_i', 'beta', 
                      'mu_theta0', 'mu_pi_i', 'mu_beta']
        ...
        try:
            ...
            divergences = sum(
                np.sum(sp['divergent__']) for sp in sampler_params
            )
        ...
        for param, r_hat in r_hat_dict.items():
            if r_hat > r_hat_threshold:
                converged = False
                warnings_list.append(
                    f"{param} R-hat = {r_hat:.4f} > {r_hat_threshold}"
                )
        ...
        for param, ess in ess_dict.items():
            if ess < ess_threshold:
                warnings_list.append(
                    f"{param} ESS = {ess:.0f} < {ess_threshold}"
                )
...
class JointParameterFitter:
    ...
    def fit_all_subjects(
        ...
        **kwargs
        ...
        population_params = self.extractor.extract_population_parameters(
            fit_result
        )
    ...
    def refit_with_more_iterations(
        self,
        previous_fit: FitResults,
        additional_iter: int = 2000
    ) -> FitResults:
        """
