"""
Sample size validation and power reporting for IPI falsification testing.
...
power in IPI framework validation studies, with special requirements for
...
class TestRequirement(Enum):
    """Specific test requirements for IPI framework."""
...
class SampleSizeValidator:
    """
    Comprehensive sample size validation for IPI framework studies.
    
    Provides validation against specific requirements for different types
    of IPI falsification tests, with special attention to soma-bias testing
    ...
    def __init__(self, 
        ...
        Args:
            ...
            strict_mode: Whether to apply strict IPI framework requirements
        """
        self.alpha = alpha
        ...
        # Define IPI-specific requirements
        self.requirements = self._define_ipi_requirements()
    ...
    def validate_soma_bias_sample_size(self, 
        ...
        if not meets_minimum:
            warnings_list.append(f"Sample size ({n_participants}) below IPI framework minimum (>{requirement.minimum_n})")
    ...
    def generate_power_report(self, 
                            ...
                            study_id: str = "IPI_Study") -> PowerReport:
        """
        ...
        if not overall_adequacy:
            recommendations.append("Study design requires revision to meet IPI framework standards")
    ...
    def _define_ipi_requirements(self) -> Dict[TestRequirement, SampleSizeRequirement]:
        """Define IPI framework-specific sample size requirements."""
        return {
            TestRequirement.SOMA_BIAS_MINIMUM: SampleSizeRequirement(
                ...
                justification="IPI framework requires n > 100 for soma-bias testing to detect interoceptive vs exteroceptive differences"
            ),
