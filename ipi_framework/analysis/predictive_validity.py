"""
Predictive validity testing framework for IPI parameters.
...
class ValidityResult:
    ...
    def __repr__(self) -> str:
        ...
        return (f"ValidityResult({self.parameter_name} → {self.task_name}: "
                f"r={self.correlation:.3f} {sig}, n={self.n_participants})")


@dataclass
class ComparativeValidityResult:
    """Results comparing IPI parameters to traditional measures."""
    ipi_parameter: str
    traditional_measure: str
    ipi_correlation: float
...
class EmotionalInterferenceTask:
    ...
    def compute_interference_score(
        self,
        emotional_rt: np.ndarray,
        neutral_rt: np.ndarray
    ) -> float:
        """
...
class ContinuousPerformanceTask:
    ...
    def predict_from_theta0(
        self,
        theta0: float,
        baseline_lapse_rate: float = 0.05,
...
class PredictivePowerComparator:
    """
    Compares predictive power of IPI parameters against traditional measures.
    """
    ...
    def test_predictive_validity(
        ...
        for est in parameter_estimates:
            ...
            matching_perf = [
                p for p in task_performance 
                if p.participant_id == est.participant_id
            ]
    ...
    def compare_to_traditional_measures(
        ...
        """
        Compare IPI parameter to traditional measure.
        
        Args:
            ipi_parameter: IPI parameter values
            traditional_measure: Traditional measure values
            outcome: Outcome variable
            ipi_name: Name of IPI parameter
        ...
        ipi_r, _, ipi_r2 = PredictivePowerComparator.compute_correlation(
            ipi_parameter, outcome
        )
        ...
        ipi_better = abs(ipi_r) > abs(trad_r)
        
        # Compute improvement percentage
        if abs(trad_r) > 0:
            improvement = ((abs(ipi_r) - abs(trad_r)) / abs(trad_r)) * 100
        else:
            improvement = 0.0
        
        return ComparativeValidityResult(
            ipi_parameter=ipi_name,
            traditional_measure=traditional_name,
            ipi_correlation=ipi_r,
            traditional_correlation=trad_r,
            ipi_r_squared=ipi_r2,
            traditional_r_squared=trad_r2,
            ipi_better=ipi_better,
            improvement_percentage=improvement
        )
    
    @staticmethod
    def generate_comparison_report(
        comparisons: List[ComparativeValidityResult]
    ) -> str:
        """
        Generate report comparing IPI to traditional measures.
        ...
        for comp in comparisons:
            symbol = "✓" if comp.ipi_better else "✗"
            report.append(f"{symbol} {comp.ipi_parameter} vs {comp.traditional_measure}")
            report.append(f"  IPI: r = {comp.ipi_correlation:.3f}, R² = {comp.ipi_r_squared:.3f}")
            report.append(f"  Traditional: r = {comp.traditional_correlation:.3f}, "
        ...
        # Summary
        n_better = sum(1 for c in comparisons if c.ipi_better)
        report.append("-" * 70)
        report.append(f"Summary: IPI parameters outperformed traditional measures "
...
class PredictiveValidityFramework:
    """
    Complete framework for testing predictive validity of IPI parameters.
    """
    ...
    def generate_validity_report(
        ...
        for key, result in validity_results.items():
            sig_symbol = "***" if result.p_value < 0.001 else \
