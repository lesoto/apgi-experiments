"""
Replication tracking and power analysis for IPI falsification testing.
...
sample sizes and statistical power in IPI framework validation studies.
"""
...
class ReplicationTracker:
    ...
    for IPI framework validation studies.
    """
    ...
    def evaluate_replication_success(self, 
                                   original_effect_size: float,
                                   replication_tolerance: float = 0.5) -> ReplicationSummary:
        """
        ...
        for exp in self.experiments:
            ...
            if (exp.p_value < self.success_threshold and 
                abs(exp.effect_size - original_effect_size) <= replication_tolerance * abs(original_effect_size)):
                successful += 1
    ...
    def _calculate_confidence_in_effect(self, 
        ...
        magnitude_factor = min(1.0, abs(np.mean(effect_sizes)) / abs(original_effect_size))
    ...
    def _interpret_replication_results(self, 
                                     success_rate: float,
                                     mean_effect_size: float,
                                     original_effect_size: float,
                                     combined_p_value: float) -> str:
        ...
        effect_ratio = abs(mean_effect_size / original_effect_size) if original_effect_size != 0 else 0
...
class PowerAnalyzer:
    ...
    statistical tests used in IPI framework validation studies.
    """
    ...
    def anova_power(self, 
                   ...
                   power: Optional[float] = None) -> PowerAnalysisResult:
        """
    ...
    def correlation_power(self, 
        ...
        if sample_size is not None:
            ...
            # Power calculation
            power_calc = 1 - norm.cdf(z_crit - abs(z_r) / se_z) + norm.cdf(-z_crit - abs(z_r) / se_z)
    ...
    def _calculate_sample_size_t_test(self, effect_size: float, power: float, test_type: str) -> int:
    ...
    def _calculate_sample_size_correlation(self, effect_size: float, power: float) -> int:
        ...
        for _ in range(1000):
            ...
            current_power = 1 - norm.cdf(z_crit - abs(z_r) / se_z) + norm.cdf(-z_crit - abs(z_r) / se_z)
    ...
    def _generate_power_recommendations(self, power: float, sample_size: int, 
                                      effect_size: float, test_type: str) -> List[str]:
    ...
    def _interpret_power_analysis(self, power: float, sample_size: int, 
                                 effect_size: float, test_type: str) -> str:
        ...
        return (f"{test_type} with {effect_magnitude} effect size (d={effect_size:.3f}) "
