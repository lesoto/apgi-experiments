...
Implements rapid 30-minute assessment battery for extracting individual IPI parameters
...
class ClinicalParameters:
    """Individual IPI parameters extracted from assessment."""
    # Core IPI parameters
...
class ReliabilityMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'test_retest_interval': str(self.test_retest_interval) if self.test_retest_interval else None,
...
class ClinicalParameterExtractor:
    """
    Clinical parameter extraction system for rapid IPI assessment.
    ...
    def create_standard_battery(self) -> AssessmentBattery:
        ...
        battery.add_task(AssessmentTask(
        ...
        logger.info(f"Created standard battery with {len(battery.tasks)} tasks, "
                   f"total duration: {battery.get_total_planned_duration():.1f} minutes")
        
        return battery
    
    def extract_parameters_from_battery(self,
                                       battery: AssessmentBattery,
                                       behavioral_data: Dict[str, Any],
                                       neural_data: Optional[Dict[str, Any]] = None,
                                       physiological_data: Optional[Dict[str, Any]] = None) -> ClinicalParameters:
        """
        Extract IPI parameters from completed assessment battery.
        ...
        logger.info(f"Extracted parameters: θₜ={params.theta_t:.2f}, "
                   f"Πₑ={params.pi_e:.2f}, Πᵢ={params.pi_i:.2f}, β={params.beta:.2f}")
        
        return params
    
    def _extract_threshold(self,
                          behavioral_data: Dict[str, Any],
                          neural_data: Optional[Dict[str, Any]]) -> Tuple[float, Tuple[float, float]]:
        ...
        # Convert to IPI threshold scale (arbitrary units)
    ...
    def _extract_exteroceptive_precision(self,
                                        behavioral_data: Dict[str, Any],
                                        neural_data: Optional[Dict[str, Any]]) -> Tuple[float, Tuple[float, float]]:
    ...
    def _extract_interoceptive_precision(self,
                                        behavioral_data: Dict[str, Any],
                                        physiological_data: Optional[Dict[str, Any]]) -> Tuple[float, Tuple[float, float]]:
    ...
    def _extract_somatic_bias(self,
                             behavioral_data: Dict[str, Any],
    ...
    def calculate_reliability_metrics(self,
                                      ...
                                      time_interval: Optional[timedelta] = None) -> ReliabilityMetrics:
        """
    ...
    def calculate_internal_consistency(self,
                                      battery: AssessmentBattery,
                                      trial_data: Dict[str, List[float]]) -> float:
        """
    ...
    def calculate_split_half_reliability(self,
                                        battery: AssessmentBattery,
                                        trial_data: Dict[str, List[float]]) -> float:
        """
    ...
    def validate_against_criterion(self,
                                   params: ClinicalParameters,
                                   criterion_measures: Dict[str, float]) -> Dict[str, float]:
        """
    ...
    def calculate_classification_metrics(self,
                                        ...
                                        predicted_labels: List[str]) -> Tuple[float, float, float]:
        """
        ...
        logger.info(f"Classification metrics - Sensitivity: {sensitivity:.3f}, "
    ...
    def generate_clinical_report(self,
                                params: ClinicalParameters,
                                reliability: Optional[ReliabilityMetrics] = None) -> str:
        """
        ...
        report = []
        report.append("=" * 60)
        report.append("IPI CLINICAL PARAMETER EXTRACTION REPORT")
        report.append("=" * 60)
        report.append("")
        ...
        # Core parameters
        report.append("CORE IPI PARAMETERS")
        report.append("-" * 60)
        report.append(f"Ignition Threshold (θₜ):        {params.theta_t:.2f} "
                     f"[95% CI: {params.theta_t_ci[0]:.2f}-{params.theta_t_ci[1]:.2f}]")
        report.append(f"Exteroceptive Precision (Πₑ):   {params.pi_e:.2f} "
                     f"[95% CI: {params.pi_e_ci[0]:.2f}-{params.pi_e_ci[1]:.2f}]")
        report.append(f"Interoceptive Precision (Πᵢ):   {params.pi_i:.2f} "
                     f"[95% CI: {params.pi_i_ci[0]:.2f}-{params.pi_i_ci[1]:.2f}]")
        report.append(f"Somatic Bias Weight (β):        {params.beta:.2f} "
    ...
    def track_longitudinal_changes(self,
                                   parameter_history: List[ClinicalParameters]) -> Dict[str, Any]:
        """
...
def extract_parameters_quick(participant_id: str,
                            behavioral_data: Dict[str, Any],
                            neural_data: Optional[Dict[str, Any]] = None,
                            physiological_data: Optional[Dict[str, Any]] = None) -> ClinicalParameters:
    """
