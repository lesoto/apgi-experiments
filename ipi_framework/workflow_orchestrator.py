"""
End-to-End Workflow Orchestration for IPI Framework Falsification Testing.
...
class WorkflowOrchestrator:
    ...
    def run_complete_workflow(self, workflow_id: Optional[str] = None) -> WorkflowResult:
        ...
        try:
            ...
            if self.config.run_secondary_tests:
                self._execute_stage(WorkflowStage.SECONDARY_FALSIFICATION, self._run_secondary_tests)
            
            if self.config.run_statistical_analysis:
                self._execute_stage(WorkflowStage.STATISTICAL_ANALYSIS, self._run_statistical_analysis)
            ...
            self.current_workflow.end_time = datetime.now()
            raise IPIFrameworkError(f"Workflow execution failed: {e}")
    ...
    def run_parallel_workflow(self, workflow_id: Optional[str] = None) -> WorkflowResult:
        ...
        try:
            ...
            self.current_workflow.overall_status = WorkflowStatus.FAILED
            self.current_workflow.end_time = datetime.now()
            raise IPIFrameworkError(f"Parallel workflow execution failed: {e}")
    ...
    def _run_initialization(self) -> Dict[str, Any]:
        ...
        ipi_params = self.controller.config_manager.get_ipi_parameters()
        exp_config = self.controller.config_manager.get_experimental_config()
        
        return {
            'system_initialized': True,
            'ipi_parameters': ipi_params.__dict__,
    ...
    def _run_secondary_tests(self) -> Dict[str, Any]:
        ...
        # Run soma-bias test
        soma_bias_result = tests['soma_bias'].run_test(n_participants=self.config.secondary_test_participants)
    ...
    def _run_statistical_analysis(self) -> Dict[str, Any]:
        ...
        for test_name, result in self.current_workflow.test_results.items():
            if hasattr(result, 'is_falsified'):
                ...
                statistical_summary['effect_sizes'][test_name] = getattr(result, 'effect_size', None)
                statistical_summary['p_values'][test_name] = getattr(result, 'p_value', None)
                statistical_summary['confidence_levels'][test_name] = getattr(result, 'confidence_level', None)
        
        # Determine overall falsification conclusion
        falsification_count = sum(1 for falsified in statistical_summary['falsification_results'].values() if falsified)
        total_tests = len(statistical_summary['falsification_results'])
        
        if falsification_count > 0:
            self.current_workflow.falsification_conclusion = f"Framework falsified by {falsification_count}/{total_tests} tests"
        else:
            self.current_workflow.falsification_conclusion = "Framework not falsified by any tests"
        
        # Calculate overall confidence
        confidence_values = [c for c in statistical_summary['confidence_levels'].values() if c is not None]
    ...
    def _run_result_aggregation(self) -> Dict[str, Any]:
        ...
        aggregated_results = {
            'workflow_summary': {
                ...
                'total_duration': str(self.current_workflow.total_duration) if self.current_workflow.total_duration else None,
                'stages_completed': len([s for s in self.current_workflow.stage_results.values() if s.status == WorkflowStatus.COMPLETED]),
    ...
    def _save_summary_report(self, file_path: Path) -> None:
        ...
        with open(file_path, 'w') as f:
            f.write("IPI Framework Falsification Testing - Workflow Summary\n")
            f.write("=" * 60 + "\n\n")
            ...
            for test_name, result in self.current_workflow.test_results.items():
                ...
                if hasattr(result, 'is_falsified'):
                    ...
                    f.write(f"{status} (confidence: {getattr(result, 'confidence_level', 'N/A')})\n")
                else:
                    f.write(f"{result}\n")
