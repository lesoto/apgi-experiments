...
protocol management and operator notifications for IPI experiments.
"""
...
class SignalQualityMonitor:
    ...
    def update_quality_metrics(self, eeg_metrics: Dict[str, Any],
                               pupil_metrics: Dict[str, Any],
                               cardiac_metrics: Dict[str, Any]) -> MultiModalQualityMetrics:
        """
        ...
        alerts = self._check_quality_thresholds(
            eeg_quality, eeg_artifact_rate, eeg_bad_channels,
            pupil_quality, pupil_data_loss, pupil_tracking_confidence,
            cardiac_quality, cardiac_sqi, cardiac_ectopic_rate,
            overall_quality
        )
        ...
        suggested_actions = self._generate_suggestions(
            eeg_quality, eeg_artifact_rate, eeg_bad_channels,
            pupil_quality, pupil_data_loss,
            cardiac_quality, cardiac_sqi
        )
    ...
    def _check_quality_thresholds(self, eeg_quality: float, eeg_artifact_rate: float,
                                 eeg_bad_channels: int, pupil_quality: float,
                                 pupil_data_loss: float, pupil_tracking_confidence: float,
                                 cardiac_quality: float, cardiac_sqi: float,
                                 cardiac_ectopic_rate: float, overall_quality: float) -> List[QualityAlert]:
    ...
    def _generate_suggestions(self, eeg_quality: float, eeg_artifact_rate: float,
                            eeg_bad_channels: int, pupil_quality: float,
                            pupil_data_loss: float, cardiac_quality: float,
                            cardiac_sqi: float) -> List[str]:
...
class ArtifactDetector:
    ...
    def detect_multi_modal_artifacts(self, eeg_artifacts: Optional[np.ndarray],
        ...
        result = {
            ...
            'eeg_artifacts': eeg_artifacts if eeg_artifacts is not None else np.zeros(len(timestamps), dtype=bool),
            'pupil_artifacts': pupil_artifacts if pupil_artifacts is not None else np.zeros(len(timestamps), dtype=bool),
            'cardiac_artifacts': cardiac_artifacts if cardiac_artifacts is not None else np.zeros(len(timestamps), dtype=bool)
        }
