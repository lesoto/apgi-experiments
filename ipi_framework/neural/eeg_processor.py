...
for P3b and HEP components in IPI experiments.
"""
...
class EEGProcessor:
    ...
    def __init__(self, sampling_rate: float = 1000.0,
                 highpass: float = 0.1, lowpass: float = 30.0,
    ...
    def _design_filters(self):
        ...
        self.bp_b, self.bp_a = signal.butter(
            4, [self.highpass / nyquist, self.lowpass / nyquist],
            btype='bandpass'
        )
    ...
    def process_realtime(self, raw_data: np.ndarray,
                        timestamps: np.ndarray,
                        channels: List[str]) -> ProcessedEEG:
        """
    ...
    def apply_reference(self, data: np.ndarray,
...
class FASTERArtifactDetector:
    ...
    def detect_bad_epochs(self, epochs: np.ndarray) -> np.ndarray:
        ...
        bad_epochs = ((variance_z > self.z_threshold) |
    ...
    def detect_bad_samples(self, data: np.ndarray,
    ...
    def clean(self, data: np.ndarray,
             ...
             detect_samples: bool = True) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
...
class EpochExtractor:
    ...
    def extract_epochs(self, data: np.ndarray,
...
class ERPExtractor:
    ...
    def extract_p3b(self, epochs: np.ndarray,
                   channel_idx: int,
                   time_window: Tuple[float, float] = (0.25, 0.5),
                   tmin: float = -0.2) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def extract_hep(self, data: np.ndarray,
                   r_peak_times: np.ndarray,
                   timestamps: np.ndarray,
                   channel_indices: Optional[List[int]] = None,
                   time_window: Tuple[float, float] = (0.25, 0.4)) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def extract_features(self, data: np.ndarray,
                        ...
                        timestamps: np.ndarray,
                        pz_channel_idx: int,
                        r_peak_times: Optional[np.ndarray] = None) -> ERPFeatures:
        """
        ...
        if r_peak_times is not None and len(r_peak_times) > 0:
            hep_epochs, hep_amps = self.extract_hep(
                data, r_peak_times, timestamps
            )
