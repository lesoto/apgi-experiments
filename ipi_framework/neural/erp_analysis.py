...
and single-trial ERP estimation for IPI framework validation.
"""
...
class ERPAnalysis:
    """
    Event-Related Potential analysis for IPI framework.
    ...
    def baseline_correct(self, data: np.ndarray, 
                        baseline_window: Tuple[float, float] = (-200, 0)) -> np.ndarray:
        """
    ...
    def apply_filter(self, data: np.ndarray, 
        ...
        if highpass and lowpass:
            ...
            # Lowpass only
            sos = signal.butter(order, lowpass / nyquist, 
                              btype='low', output='sos')
        elif highpass:
            # Highpass only
            sos = signal.butter(order, highpass / nyquist, 
    ...
    def detect_peak(self, data: np.ndarray, 
                   search_window: Tuple[float, float],
                   polarity: str = 'positive') -> Tuple[float, float, int]:
        """
    ...
    def extract_p3b(self, data: np.ndarray, 
                   time_zero_idx: int,
                   search_window: Tuple[float, float] = (300, 600)) -> P3bMetrics:
        """
    ...
    def extract_early_components(self, data: np.ndarray,
                                time_zero_idx: int) -> Dict[str, Tuple[float, float]]:
        """
    ...
    def extract_all_components(self, data: np.ndarray,
                              time_zero_idx: int,
                              channel: Optional[str] = None) -> ERPComponents:
        """
    ...
    def single_trial_estimation(self, trial_data: np.ndarray,
                               time_zero_idx: int,
    ...
    def _adaptive_filtering(self, data: np.ndarray, time_zero_idx: int) -> np.ndarray:
        ...
        if time_zero_idx > 100:
            ...
            # Soft thresholding
            filtered = np.where(np.abs(data) > threshold, 
                              data - np.sign(data) * threshold,
                              0)
            return filtered
    ...
    def compute_grand_average(self, trials: List[np.ndarray],
                            reject_threshold: Optional[float] = None) -> Tuple[np.ndarray, int]:
        """
    ...
    def bootstrap_confidence_interval(self, trials: List[np.ndarray],
