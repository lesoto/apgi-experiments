...
and phase-amplitude coupling detection for IPI framework validation.
"""
...
class GammaSynchronyAnalysis:
    """
    Gamma-band synchrony analysis for IPI framework.
    ...
    def extract_gamma_band(self, data: np.ndarray,
        ...
        sos = signal.butter(4, [low / self.nyquist, high / self.nyquist],
    ...
    def compute_coherence(self, signal1: np.ndarray, 
                         signal2: np.ndarray,
                         nperseg: int = 256,
        ...
        # Compute coherence
        freqs, coherence = signal.coherence(signal1, signal2,
    ...
    def compute_gamma_coherence(self, signal1: np.ndarray,
                               signal2: np.ndarray,
                               channel_pair: Optional[Tuple[str, str]] = None) -> CoherenceMetrics:
        """
    ...
    def _compute_phase_lag(self, signal1: np.ndarray,
                          signal2: np.ndarray,
        ...
        freqs_csd, csd = signal.csd(signal1, signal2,
                                    fs=self.sampling_rate,
                                    nperseg=256)
    ...
    def _compute_phase_locking_value(self, signal1: np.ndarray,
    ...
    def compute_phase_amplitude_coupling(self, 
        ...
        """
        # Filter signals
        sos_phase = signal.butter(4, [phase_band[0] / self.nyquist, 
                                     phase_band[1] / self.nyquist],
                                btype='band', output='sos')
        phase_signal = signal.sosfiltfilt(sos_phase, low_freq_signal)
        
        sos_amp = signal.butter(4, [amp_band[0] / self.nyquist,
                                   amp_band[1] / self.nyquist],
                              btype='band', output='sos')
    ...
    def compute_cross_frequency_coupling(self, data: np.ndarray,
                                        phase_freqs: List[float] = [4, 8, 13],
                                        amp_freqs: List[float] = [30, 50, 70]) -> np.ndarray:
        """
    ...
    def compute_frontal_posterior_coherence(self, data: np.ndarray,
                                           frontal_channels: List[int],
                                           posterior_channels: List[int]) -> float:
        """
    ...
    def compute_connectivity_matrix(self, data: np.ndarray,
    ...
    def analyze_network_connectivity(self, data: np.ndarray,
                                    channel_names: List[str],
                                    frontal_channels: List[int],
                                    posterior_channels: List[int],
                                    left_channels: Optional[List[int]] = None,
                                    right_channels: Optional[List[int]] = None) -> NetworkConnectivity:
        """
    ...
    def _compute_clustering_coefficient(self, connectivity: np.ndarray,
    ...
    def _compute_path_length(self, connectivity: np.ndarray,
                            threshold: float = 0.5) -> float:
        """
    ...
    def detect_gamma_bursts(self, data: np.ndarray,
                           threshold: float = 2.0,
                           min_duration: float = 50.0) -> List[Dict]:
        """
