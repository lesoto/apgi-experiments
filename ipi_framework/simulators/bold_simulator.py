"""
BOLD activation simulator for IPI Framework falsification testing.
...
class BOLDSimulator:
    ...
    def __init__(self, random_seed: Optional[int] = None):
        ...
        self.bilateral_pairs = [
            ...
            (BrainRegion.POSTERIOR_PARIETAL_LEFT.value, BrainRegion.POSTERIOR_PARIETAL_RIGHT.value),
            (BrainRegion.FRONTAL_EYE_FIELDS_LEFT.value, BrainRegion.FRONTAL_EYE_FIELDS_RIGHT.value)
        ]
    
    def generate_conscious_signature(self,
                                   z_range: Tuple[float, float] = (3.5, 8.0),
                                   noise_level: float = 0.3,
                                   bilateral_coherence: float = 0.8) -> BOLDSignature:
        """
        ...
        for left_region, right_region in self.bilateral_pairs:
            if left_region in activations and right_region in activations:
                ...
                coherence_factor = self.rng.uniform(bilateral_coherence - 0.1, 
                                                   bilateral_coherence + 0.1)
                right_base = left_z * coherence_factor
                right_noise = self.rng.normal(0, noise_level * 0.5)
                activations[right_region] = max(right_base + right_noise, 
                                              self.conscious_z_threshold + 0.1)
    ...
    def generate_unconscious_signature(self,
                                     z_range: Tuple[float, float] = (0.5, 1.8),
                                     noise_level: float = 0.2) -> BOLDSignature:
        """
        ...
        all_regions = (self.core_consciousness_regions + 
                      [r for r in self.frontoparietal_network 
    ...
    def generate_signature(self,
                          ...
                          target_z: float,
                          noise_level: float = 0.25) -> BOLDSignature:
        """
    ...
    def generate_ai_acc_suppressed_signature(self,
                                           dlpfc_z_range: Tuple[float, float] = (3.5, 7.0),
                                           ai_acc_z_range: Tuple[float, float] = (0.2, 1.5),
                                           noise_level: float = 0.3) -> BOLDSignature:
        """
        ...
        other_regions = [r for r in self.frontoparietal_network 
                        if r not in dlpfc_ips_regions]
    ...
    def _create_signature(self, activations: Dict[str, float]) -> BOLDSignature:
        ...
        active_regions = [region for region, z in activations.items() 
                         if z > self.conscious_z_threshold]
        
        # Check bilateral DLPFC activation
        bilateral_dlpfc = (
            activations.get(BrainRegion.DLPFC_LEFT.value, 0) > self.conscious_z_threshold and
            activations.get(BrainRegion.DLPFC_RIGHT.value, 0) > self.conscious_z_threshold
        )
    ...
    def _calculate_network_coherence(self, activations: Dict[str, float]) -> float:
        ...
        network_activations = [activations.get(region, 0.0) 
                              for region in self.frontoparietal_network]
