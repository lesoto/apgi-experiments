...
if torch.cuda.is_available():
    ...
    cap = None
    repo = "kernels-community/flash-attn3"  # Use community version for CPU
    autocast_ctx = torch.amp.autocast(device_type="cpu", dtype=torch.float32)
    H100_BF16_PEAK_FLOPS = 1e10  # Placeholder for CPU
    use_flash_attn = False
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # Value residual: mix in value embedding with
...
class GPT(nn.Module):
    ...
    def setup_optimizer(
        ...
        print(
            f"Scaling AdamW LRs by " f"1/sqrt({model_dim}/768) = {dmodel_lr_scale:.6f}"
        )
        param_groups = [
            dict(
                kind="adamw",
