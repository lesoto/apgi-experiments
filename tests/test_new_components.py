...
try:
    from apgi_framework.research.cross_species_validation import (
        CrossSpeciesValidator, Species, NeuralSignatureType, CrossSpeciesSignature, ValidationResult
    )
...
try:
    from research.clinical_biomarkers.experiments.biomarker_analysis import (
        BiomarkerAnalyzer, ClinicalCondition, BiomarkerType, PatientProfile, ClinicalBiomarker
    )
...
try:
    from apgi_framework.research.threshold_detection_paradigm import (
        ThresholdDetectionSystem, ModalityType, ThresholdMethod, StimulusParameters, TrialResponse
    )
...
class TestClinicalBiomarkers:
    ...
    def test_feature_biomarker_creation(self):
        ...
        biomarker = analyzer._create_feature_biomarker(
            "p3b_amplitude_extero", case_values, control_values,
            ClinicalCondition.MAJOR_DEPRESSION, BiomarkerType.NEURAL_SIGNATURE
        )
    ...
    def test_biomarker_validation(self):
        ...
        # Add a feature
        from research.clinical_biomarkers.experiments.biomarker_analysis import BiomarkerFeature
        ...
        validation_patients = [
            PatientProfile(
                ...
                age=30 + i,
                sex="F",
                clinical_condition=ClinicalCondition.MAJOR_DEPRESSION if i < 10 else ClinicalCondition.HEALTHY_CONTROL,
...
class TestThresholdDetectionParadigm:
    ...
    def test_adaptive_staircase(self):
        ...
        from apgi_framework.research.threshold_detection_paradigm import AdaptiveStaircase
    ...
    def test_threshold_estimation(self):
        ...
        assert estimate.confidence_interval[0] <= estimate.threshold_value <= estimate.confidence_interval[1]
    ...
    def test_confidence_interval_calculation(self):
        ...
        ci_lower, ci_upper = system._calculate_confidence_interval(intensities, responses, threshold)
...
@pytest.mark.skipif(not (CROSS_SPECIES_AVAILABLE and CLINICAL_BIOMARKERS_AVAILABLE), 
...
class TestIntegration:
    ...
    @pytest.mark.skipif(not (CLINICAL_BIOMARKERS_AVAILABLE and THRESHOLD_DETECTION_AVAILABLE), 
