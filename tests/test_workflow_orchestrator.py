...
class TestWorkflowResult:
    ...
    def test_workflow_result_properties(self):
        ...
        stage2 = WorkflowStageResult(
            WorkflowStage.SYSTEM_VALIDATION, WorkflowStatus.COMPLETED, start_time, end_time
        )
        ...
        with mock.patch.object(self.orchestrator, '_run_initialization') as mock_init:
            with mock.patch.object(self.orchestrator, '_run_system_validation') as mock_validation:
                with mock.patch.object(self.orchestrator, '_run_primary_tests') as mock_primary:
                    with mock.patch.object(self.orchestrator, '_run_statistical_analysis') as mock_analysis:
                        with mock.patch.object(self.orchestrator, '_run_result_aggregation') as mock_aggregation:
                            with mock.patch.object(self.orchestrator, '_run_report_generation') as mock_report:
                                with mock.patch.object(self.orchestrator, '_run_cleanup') as mock_cleanup:
...
class TestModuleFunctions:
    ...
    def test_run_standard_falsification_workflow(self):
        ...
        with mock.patch('apgi_framework.workflow_orchestrator.WorkflowOrchestrator') as mock_orchestrator_class:
