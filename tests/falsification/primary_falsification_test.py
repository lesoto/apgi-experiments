...
class PrimaryFalsificationTest:
    ...
    def run_falsification_test(
        ...
        try:
            ...
            logger.info(
                f"Primary Falsification Test completed successfully in {(end_time - start_time).total_seconds():.2f}s"
            )
            ...
            end_time = datetime.now()
            logger.error(
                f"Primary Falsification Test failed after {(end_time - start_time).total_seconds():.2f}s: {str(e)}"
            )
            raise
