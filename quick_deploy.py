...
class QuickDeploy:
    ...
    def show_success_info(self):
        ...
        print(f"🌐 Web Interface: http://localhost:{list(self.manager.config.ports.values())[0]}")
    ...
    def show_logs(self):
        ...
        try:
            subprocess.run([
                "docker", "logs", "-f", self.manager.config.container_name
            ])
...
def main():
    ...
    if len(sys.argv) < 2 or sys.argv[1] == "deploy":
        ...
        print("=" * 50)
        print("This script will help you deploy APGI Framework with minimal configuration.")
        print()
