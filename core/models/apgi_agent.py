...
class APGIAgent:
    ...
    def _calculate_surprise(self, t: int):
        ...
        self.S[t] = self.config.Pi_e * abs(self.eps_e[t]) + self.Pi_i[t] * abs(
            self.eps_i[t]
        )
    ...
    def run(self):
        ...
        for t in range(1, self.config.T):
            ...
            self.body_state[t] = self.config.body_setpoint + np.random.normal(
                0, self.config.body_noise_sd
            )
    ...
    def plot_signals(self):
        ...
        axs[0].axvline(
            self.context_onset, color="gray", linestyle="--", label="Context Onset"
        )
        axs[0].legend()
        ...
        axs[3].plot(
            theta_curve, color="red", linestyle="--", label="Threshold $\\theta_t$"
        )
