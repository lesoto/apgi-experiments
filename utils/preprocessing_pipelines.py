...
class PupilPreprocessor:
    ...
    def _interpolate_blinks(self, df: pd.DataFrame, pupil_column: str) -> pd.DataFrame:
        ...
        if len(blink_indices) > 0:
            ...
            df_processed.loc[:, pupil_column] = df_processed[pupil_column].interpolate(
                method="linear"
            )
...
def main():
    ...
    if demo_file.exists():
        ...
        print("Creating sample demo data for demonstration...")
        ...
        n_samples = 1000
        sample_data = {
            "timestamp": np.linspace(0, 10, n_samples),
            "eeg_fz": np.random.randn(n_samples) * 10,
            "eeg_cz": np.random.randn(n_samples) * 10,
            "eeg_pz": np.random.randn(n_samples) * 10,
            "pupil_diameter": np.random.normal(4, 1, n_samples),
            "eda": np.random.exponential(0.5, n_samples),
            "heart_rate": np.random.normal(70, 10, n_samples),
        }

        # Add some realistic artifacts
        sample_df = pd.DataFrame(sample_data)
        sample_df.to_csv(demo_file, index=False)

        print(f"✅ Created demo file: {demo_file}")
        print("You can run this script again to process the demo data.")
