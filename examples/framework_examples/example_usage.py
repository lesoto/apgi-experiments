...
def example_gamma_synchrony():
    ...
    signal2 = (
        np.sin(2 * np.pi * gamma_freq * time + phase_offset)
    ...
    metrics = gamma.compute_gamma_coherence(
        signal1=signal1, signal2=signal2, channel_pair=("Frontal", "Posterior")
    )
    ...
    # Detect gamma bursts
    bursts = gamma.detect_gamma_bursts(data=signal1, threshold=2.0, min_duration=50.0)

    print(f"Detected {len(bursts)} gamma bursts")
    if bursts:
        print(
            f"  First burst: {bursts[0]['start_time']:.1f} - {bursts[0]['end_time']:.1f} ms"
        )
...
def example_integrated_pipeline():
    ...
    for trial in range(n_trials):
        ...
        # Extract P3b
        components = erp.extract_all_components(
            data=trial_data[22], time_zero_idx=200, channel="Pz"
        )
        ...
        fp_coherence = gamma.compute_frontal_posterior_coherence(
            data=trial_data, frontal_channels=[0, 1, 2], posterior_channels=[20, 21, 22]
        )
    ...
    print(
        f"  Mean P3b amplitude: {np.mean(all_p3b_amplitudes):.2f} ± {np.std(all_p3b_amplitudes):.2f} µV"
    )
    print(
        f"  Mean gamma coherence: {np.mean(all_gamma_coherences):.3f} ± {np.std(all_gamma_coherences):.3f}"
    )
