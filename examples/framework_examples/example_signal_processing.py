...
def example_eeg_processing():
    ...
    eeg_processor = EEGProcessor(
        sampling_rate=1000.0, highpass=0.1, lowpass=30.0, notch_freq=60.0
    )
    ...
    print(
        f"Processed {processed.data.shape[1]} samples from {processed.data.shape[0]} channels"
    )
    ...
    features = erp_extractor.extract_features(
        cleaned_data, event_times, timestamps, pz_channel_idx
    )

    print(
        f"P3b amplitude: {features.p3b_amplitude:.2f} µV"
        if features.p3b_amplitude
        else "P3b: Not detected"
    )
...
def example_pupillometry_processing():
    ...
    pupil_data[response_window] += 0.5 * np.exp(
        -(t[response_window] - stimulus_time) / 0.3
    )
...
def example_integrated_quality_control():
    ...
    def print_notification(notification):
        print(
            f"[{notification['type'].upper()}] {notification.get('message', 'Quality update')}"
        )
