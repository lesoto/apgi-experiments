...
def analyze_falsification_rates(results: List[Dict[str, Any]]):
    ...
    if len(test_types) > 1:
        ...
        for test_type, counts in test_types.items():
            ...
            logger.info(f"  {test_type}: {counts['falsified']}/{counts['total']} ({rate:.1%})")
...
def analyze_statistical_power(results: List[Dict[str, Any]]):
    ...
    power_values = [r.get('statistical_power', 0) for r in results if 'statistical_power' in r]
    ...
    logger.info(f"\nPower Distribution:")
    logger.info(f"  High power (≥0.8): {high_power} ({high_power/len(power_values):.1%})")
    logger.info(f"  Medium power (0.5-0.8): {medium_power} ({medium_power/len(power_values):.1%})")
...
def analyze_effect_sizes(results: List[Dict[str, Any]]):
    ...
    logger.info(f"  Medium (0.5 ≤ |d| < 0.8): {medium} ({medium/len(effect_sizes):.1%})")
    logger.info(f"  Large (|d| ≥ 0.8): {large} ({large/len(effect_sizes):.1%})")
    
    # Relationship between effect size and falsification
    falsified_results = [r for r in results if r.get('is_falsified', False) and 'effect_size' in r]
    not_falsified_results = [r for r in results if not r.get('is_falsified', False) and 'effect_size' in r]
...
def compare_experiments(results: List[Dict[str, Any]]):
    ...
    logger.info(f"{'Rank':<6} {'File':<30} {'Falsified':<12} {'Confidence':<12} {'P-value':<12}")
    logger.info("-" * 80)
    ...
    for idx, result in enumerate(sorted_results[:5], 1):
        ...
        logger.info(f"{idx:<6} {file_name:<30} {falsified:<12} {confidence:<12.3f} {p_value:<12.6f}")
    ...
    if falsified and not_falsified:
        ...
        logger.info(f"  Mean confidence: {np.mean([r.get('confidence_level', 0) for r in falsified]):.3f}")
        logger.info(f"  Mean effect size: {np.mean([r.get('effect_size', 0) for r in falsified]):.3f}")
        logger.info(f"  Mean p-value: {np.mean([r.get('p_value', 1.0) for r in falsified]):.6f}")
        
        logger.info(f"\nNot Falsified Experiments (n={len(not_falsified)}):")
        logger.info(f"  Mean confidence: {np.mean([r.get('confidence_level', 0) for r in not_falsified]):.3f}")
        logger.info(f"  Mean effect size: {np.mean([r.get('effect_size', 0) for r in not_falsified]):.3f}")
        logger.info(f"  Mean p-value: {np.mean([r.get('p_value', 1.0) for r in not_falsified]):.6f}")
...
def export_analysis_report(results: List[Dict[str, Any]], output_file: str = "analysis_report.json"):
    """
    ...
    report = {
        ...
        'summary': {
            'falsified_count': sum(1 for r in results if r.get('is_falsified', False)),
            'falsification_rate': sum(1 for r in results if r.get('is_falsified', False)) / len(results) if results else 0,
        },
        'statistics': {},
        'experiments': results
    }
