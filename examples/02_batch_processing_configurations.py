...
def batch_process_threshold_variations():
    ...
    try:
        ...
        logger.info("\nDetailed Results:")
        logger.info(f"{'Threshold':<12} {'Falsified':<12} {'Confidence':<12} {'P-value':<12}")
        logger.info("-" * 60)
        for r in results:
            falsified_str = "YES" if r['is_falsified'] else "NO"
            logger.info(f"{r['threshold']:<12.1f} {falsified_str:<12} {r['confidence']:<12.3f} {r['p_value']:<12.6f}")
...
def batch_process_precision_combinations():
    ...
    try:
        ...
        logger.info("\nDetailed Results:")
        logger.info(f"{'Extero':<10} {'Intero':<10} {'Ratio':<10} {'Falsified':<12} {'Confidence':<12}")
        logger.info("-" * 60)
        for r in results:
            falsified_str = "YES" if r['is_falsified'] else "NO"
            logger.info(f"{r['extero_precision']:<10.1f} {r['intero_precision']:<10.1f} "
...
def batch_process_comprehensive_sweep():
    ...
    param_configs = [
        ...
        {'threshold': 2.5, 'extero_precision': 2.5, 'intero_precision': 2.0, 'steepness': 2.0},
        # Medium threshold, medium precision
        {'threshold': 3.5, 'extero_precision': 2.0, 'intero_precision': 1.5, 'steepness': 2.0},
        # High threshold, low precision
        {'threshold': 4.5, 'extero_precision': 1.5, 'intero_precision': 1.0, 'steepness': 2.0},
        # Steep sigmoid
        {'threshold': 3.5, 'extero_precision': 2.0, 'intero_precision': 1.5, 'steepness': 3.0},
        # Shallow sigmoid
        {'threshold': 3.5, 'extero_precision': 2.0, 'intero_precision': 1.5, 'steepness': 1.0},
        # High somatic gain
        {'threshold': 3.5, 'extero_precision': 2.0, 'intero_precision': 1.5, 'steepness': 2.0, 'somatic_gain': 2.0},
        # Low somatic gain
        {'threshold': 3.5, 'extero_precision': 2.0, 'intero_precision': 1.5, 'steepness': 2.0, 'somatic_gain': 0.8},
    ]
