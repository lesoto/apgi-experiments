# IPI Framework — Testable Predictions

Testable Predictions: Empirical Validation Roadmap

To guard against post-hoc rationalization, the IPI framework establishes a priori evidential standards that define what constitutes strong support, weak support, or falsification of its core predictions. These criteria apply uniformly across empirical domains and are grounded in effect size, statistical significance, and replicability. These standards anchor the IPI framework’s commitment to empirical rigor and ensure that its specific, falsifiable predictions—spanning neural dynamics, neuromodulation, and behavioral correlates—can be objectively evaluated. The following section presents a comprehensive and prioritized list of such empirical tests.
...
Failure of 5: Clinical profiles are not θₜ-mediated → disorders involve other parameters or non-IPI mechanisms
...
Hypothesis:
Individual-level fitted IPI parameters predict antidepressant treatment response.
...
Develop brief (~30 min) assessment battery extracting IPI parameters to guide treatment selection.
...
Hypothesis:
Intracranial recordings in surgical patients show layer-specific activation sequence matching IPI circuit model.
...
IPI agent: Implements Sₜ = Πₑ·εₑ + β·Πᵢ·εᵢ, threshold θₜ, somatic markers M_{c,a}
...
Validation:
Demonstrates computational advantages of IPI architecture, supporting evolutionary argument.

Prediction 5.2: Parameter Recovery from Behavior

Hypothesis:
IPI model parameters fitted to human behavioral data are stable, reliable and predict independent neural measures.
