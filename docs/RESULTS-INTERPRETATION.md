...
**Key Point**: "Not falsified" ≠ "proven true". It means the framework survived this particular challenge.
...
Start
  ...
  │   │   │   ├─ YES → p_value < 0.05?
  │   │   │   │   │
  │   │   │   │   ├─ YES → STRONG FALSIFICATION
  │   │   │   │   │        Action: Verify and replicate
  │   │   │   │   │
  │   │   │   │   └─ NO → INCONCLUSIVE
  │   │   │   │            Action: Check analysis
  │   │   │   │
  │   │   │   └─ NO → WEAK FALSIFICATION
  │   │   │            Action: Consider practical significance
  │   │   │
  │   │   └─ NO → UNCERTAIN FALSIFICATION
  │   │            Action: Increase sample size
  │   │
  │   └─ NO → confidence > 0.8?
  │       │
  │       ├─ YES → FRAMEWORK SUPPORTED
  │       │        Action: Test other criteria
  │       │
  │       └─ NO → INCONCLUSIVE
  │                Action: Increase sample size
```
...
Result Obtained
  ...
  │      4. Consider framework revision
  │
  ├─ Strong Support?
  │   └─ 1. Test other criteria
  ...
  └─ Mixed Results?
      └─ 1. Identify which tests failed
         2. Look for patterns
         3. Consider partial falsification
...
noise. The framework's core prediction that ignition is sufficient 
for consciousness is strongly supported by these results.

Implications:
These results provide strong evidence for the framework's central 
claim. The high confidence, significant p-value, medium-large effect 
size, and adequate statistical power all support the conclusion. 
Further testing with other falsification criteria is warranted.
```
...
```
"The test showed a medium effect (d = 0.62) with high confidence 
(0.87) and was highly significant (p = 0.002)"
```
...
```
"The framework survived this falsification attempt, providing 
...
```
"While statistically significant (p = 0.03), the effect was small 
(d = 0.15) and may not be practically meaningful"
```
...
```
"No significant effect was found (p = 0.12), but the test was 
underpowered (power = 0.62), so this result is inconclusive"
```
