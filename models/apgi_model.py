...
class APGIParams:
    ...
    beta: float = 1.0  # Inverse temperature for decision making
...
class APGIModel:
    """Implements the APGI (Interoceptive Predictive Integration) model."""

    def __init__(self, params: Optional[APGIParams] = None):
        self.params = params or APGIParams()
    ...
    def simulate(
        self, mc_a: Optional[float] = None
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Run the APGI model simulation.
        ...
        theta_t = np.full_like(
            S_t, self.params.theta_base
        )  # Constant threshold for now
...
def plot_simulation(
    ...
    B_t: np.ndarray,
    theta_t: np.ndarray,
    title: str = "APGI Model Simulation",
) -> None:
    """Plot the results of an APGI model simulation."""
...
def run_demo():
    """Run a demonstration of the APGI model with default parameters."""
