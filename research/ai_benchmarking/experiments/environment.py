...
class GridWorld:
    def __init__(
        ...
        self._place_objects(
            n_obstacles, ObjectType.OBSTACLE, self.obstacles, radius=0.3
        )
        self._place_objects(
            n_predators, ObjectType.PREDATOR, self.predators, energy=10.0
        )
    ...
    def _check_collision(
        ...
        position: np.ndarray,
        radius: float,
        exclude: Optional[List[GameObject]] = None,
    ) -> bool:
    ...
    def _check_predator_encounter(self, agent):
        ...
        for predator in self.predators:
            ...
            if dist < (
                agent.radius + predator.radius + 0.1
            ):  # Slightly larger radius for catching
    ...
    def render(self):
        ...
        for agent in self.agents:
            if agent.active:
                ...
                plt.text(
                    agent.position[0],
                    agent.position[1] + agent.radius + 0.2,
                    f"{agent.energy:.1f}",
                    ha="center",
                    fontsize=8,
                )
