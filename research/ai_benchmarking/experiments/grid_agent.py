...
class APGIAgent:
    ...
    def perceive_and_predict(self):
        ...
        self.prediction_error_intero = (
            abs(self.viability_setpoint - self.viability) / 100.0
        )
    ...
    def update_precision(self):
        ...
        if self.viability < 40:
            ...
            self.precision_intero = 1.0 + (
                0.5 * (1 - self.viability / 100)
            )  # Scale with need
    ...
    def check_ignition(self):
        ...
        current_cell_type = self.world[self.pos[1], self.pos[0]]
        somatic_gain_M = abs(
            self.somatic_markers[current_cell_type]
        ...
        self.ignition_threshold_theta = 5.0 + (
            2.0 if len(self.ignition_history) > 0 and self.ignition_history[-1] else 0
        )
        ...
        if self.is_ignited:
            ...
            if not was_ignited:
                print(
                    f"🔥 IGNITION! S_t={self.surprise_magnitude_S:.2f}, θ_t={self.ignition_threshold_theta:.2f}"
                )
                print(
                    f"   Details: Π^e={self.precision_extero:.2f}, ε^e={self.prediction_error_extero:.2f}"
                )
                print(
                    f"             Π^i={self.precision_intero:.2f}, ε^i={self.prediction_error_intero:.2f}"
                )
        else:
    ...
    def act(self):
        ...
        if not self.is_ignited:
            ...
            if current_cell_type == CellType.RESOURCE:
                ...
                action = self.possible_actions[
                    np.random.randint(0, len(self.possible_actions))
                ]
                self.move(action)
        ...
        # Deliberative (Conscious) Action
            ...
            if current_cell_type == CellType.RESOURCE:
                ...
                self.viability = max(
                    0, self.viability - 15
                )  # Take more damage when conscious of threat
    ...
    def consume_resource(self):
        if self.world[self.pos[1], self.pos[0]] == CellType.RESOURCE:
            ...
            self.world[self.pos[1], self.pos[0]] = (
                CellType.EMPTY
...
class Simulation:
    ...
    def draw(self):
        ...
        agent_rect = pygame.Rect(
            self.agent.pos[0] * CELL_SIZE,
            self.agent.pos[1] * CELL_SIZE,
            CELL_SIZE,
            CELL_SIZE,
        )
        agent_color = (
            (255, 0, 0) if self.agent.is_ignited else (0, 0, 255)
        )  # RED if ignited, BLUE otherwise
        ...
        pygame.draw.rect(
            self.screen, (240, 240, 240), (0, panel_y, SCREEN_SIZE, panel_height)
        )
        ...
        ignition_text = self.font.render(
            f"IGNITION: {self.agent.is_ignited} - {self.agent.conscious_content}",
            True,
            status_color,
        )
        ...
        instruct_text = self.small_font.render(
            "Press R to reset simulation", True, (100, 100, 100)
        )
            ...
            if not paused:
                ...
                if self.agent.viability <= 0:
                    death_text = self.font.render(
                        "AGENT DIED - Press R to reset", True, (255, 0, 0)
                    )
