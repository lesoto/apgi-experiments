...
class InteroceptiveGatingExperiment(BaseExperiment):
    ...
    def _update_threshold(self, condition: str, response_was_correct: bool):
        ...
        self.current_contrasts[condition] = np.clip(
            self.current_contrasts[condition], 0.01, 1.0
        )

        # Update threshold estimate (simple moving average)
        self.thresholds[condition] = (
            0.9 * self.thresholds[condition] + 0.1 * self.current_contrasts[condition]
        )

        return self.current_contrasts[condition]

    def run_trial(
        self, participant_id: int, condition: str, trial_idx: int = 0
    ) -> Dict:
        ...
        trial_data["trial_num"] = (
            len(
                [
                    d
                    for d in self.participant_data.get(participant_id, [])
    ...
    def run_block(
        self, participant_id: int, condition: str, n_trials: Optional[int] = None
    ):
...
def run_interoceptive_gating_experiment(n_participants=10, n_trials_per_condition=50):
    ...
    experiment = InteroceptiveGatingExperiment(n_participants=n_participants)
