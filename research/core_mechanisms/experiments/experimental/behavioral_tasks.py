...
class BehavioralThresholdCalculator:
    ...
    def calculate_threshold(
        self, intensities: List[float], responses: List[bool]
    ) -> Tuple[float, float]:
        """
        ...
        logger.info(
            f"Behavioral threshold: {self.threshold_estimate:.4f} ± {self.threshold_std:.4f}"
        )
        return self.threshold_estimate, self.threshold_std

    def get_psychometric_curve(self, intensities: np.ndarray) -> np.ndarray:
        """
        ...
        detection_probs = 1.0 / (
            1.0 + np.exp(-slope * (intensities - self.threshold_estimate))
        )
...
class P3bCorrelationValidator:
    ...
    def validate_threshold(
        self, threshold_estimates: List[float], p3b_amplitudes: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        is_valid = (
            abs(self.correlation) >= self.target_correlation and self.p_value < 0.05
        )
        ...
        logger.info(
            f"P3b correlation validation: r={self.correlation:.3f}, p={self.p_value:.4f}, valid={is_valid}"
        )
        return result
...
class DetectionTask:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized DetectionTask: {modality} modality, {n_trials} trials"
        )
    ...
    def initialize(self) -> bool:
        ...
        try:
            self.state_machine.transition_to(
                TaskState.INITIALIZING, "Initializing detection task"
            )
    ...
    def run_trial(self, trial_number: int) -> Optional[DetectionTrialResult]:
        ...
        try:
            ...
            self.response_collector.start_collection(
                stimulus_onset_time=stimulus_onset, response_window_ms=2000.0
            )
            ...
            trial_result = DetectionTrialResult(
                ...
                stimulus_modality=(
                    StimulusModality.VISUAL
                    if self.modality == "visual"
                    else StimulusModality.AUDITORY
                ),
            ...
            logger.debug(
                f"Trial {trial_number}: intensity={intensity:.4f}, detected={detected}, RT={reaction_time:.1f}ms"
            )
            return trial_result
    ...
    def run(self) -> bool:
        ...
        if not self.state_machine.transition_to(
            TaskState.RUNNING, "Starting detection task"
        ):
            return False
        ...
        try:
            ...
            for trial_num in range(1, self.n_trials + 1):
                ...
                if trial_num % 20 == 0:
                    ...
                    logger.info(
                        f"Progress: {trial_num}/{self.n_trials} ({progress:.1f}%)"
                    )
            ...
            self.state_machine.transition_to(
                TaskState.COMPLETED, "Detection task completed"
            )
    ...
    def _calculate_threshold(self) -> None:
        ...
        # Calculate threshold
        self.threshold_estimate, self.threshold_std = (
            self.threshold_calculator.calculate_threshold(intensities, responses)
        )
        ...
        logger.info(
            f"Threshold estimates - Behavioral: {self.threshold_estimate:.4f} ± {self.threshold_std:.4f}, "
    ...
    def get_results(self) -> Dict[str, Any]:
        ...
        reaction_times = [
            ...
            for trial in self.trials
            if trial.behavioral_response
            and trial.behavioral_response.reaction_time_valid
        ]
...
class DPrimeCalculator:
    ...
    def calculate_d_prime(
        ...
        asynchronous_responses: List[bool],
    ) -> float:
        """
        ...
        self.criterion = -0.5 * (
            norm.ppf(self.hit_rate) + norm.ppf(self.false_alarm_rate)
        )

        logger.info(
            f"d' = {self.d_prime:.3f}, criterion = {self.criterion:.3f}, "
...
class ConfidenceAnalyzer:
    ...
    def analyze_confidence(
        self, accuracies: List[bool], confidences: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        self.confidence_accuracy_correlation = np.corrcoef(accuracies, confidences)[
            0, 1
        ]

        # Calculate mean confidence for correct vs incorrect trials
        correct_confidences = [
            conf for acc, conf in zip(accuracies, confidences) if acc
        ]
        incorrect_confidences = [
            conf for acc, conf in zip(accuracies, confidences) if not acc
        ]

        self.mean_confidence_correct = (
            np.mean(correct_confidences) if correct_confidences else 0.0
        )
        self.mean_confidence_incorrect = (
        ...
        metacognitive_sensitivity = (
            self.mean_confidence_correct - self.mean_confidence_incorrect
        )
        ...
        logger.info(
            f"Confidence analysis: r={self.confidence_accuracy_correlation:.3f}, "
...
class AdaptiveAsynchronyAdjuster:
    ...
    def adjust_asynchrony(self, current_d_prime: float) -> float:
        ...
        if current_d_prime < 0.5:
            ...
            logger.info(
                f"Increasing asynchrony to {self.current_asynchrony_ms:.1f}ms (d'={current_d_prime:.3f})"
            )
            ...
            logger.info(
                f"Decreasing asynchrony to {self.current_asynchrony_ms:.1f}ms (d'={current_d_prime:.3f})"
            )
...
class HeartbeatDetectionTask:
    ...
    def initialize(self) -> bool:
        ...
        try:
            self.state_machine.transition_to(
                TaskState.INITIALIZING, "Initializing heartbeat detection task"
            )
            ...
            self.state_machine.transition_to(
                TaskState.READY, "Heartbeat detection task ready"
            )
    ...
    def run_trial(self, trial_number: int) -> Optional[HeartbeatTrialResult]:
        ...
        try:
            ...
            if is_synchronous:
                ...
                tone_delay_ms = np.random.uniform(
                    asynchrony_ms - 50, asynchrony_ms + 50
                )
            ...
            # Collect response
            self.response_collector.start_collection(
                stimulus_onset_time=stimulus_onset, response_window_ms=3000.0
            )
            ...
            if confidence is None:
                confidence = self.response_collector.collect_confidence_rating(
                    prompt="How confident are you?", scale_min=0.0, scale_max=1.0
                )
            ...
            logger.debug(
                f"Trial {trial_number}: sync={is_synchronous}, response={detected_as_sync}, "
    ...
    def run(self) -> bool:
        ...
        """
        if not self.state_machine.transition_to(
            TaskState.RUNNING, "Starting heartbeat detection task"
        ):
            return False
        ...
        try:
            ...
            # Transition to completed
            self.state_machine.transition_to(
                TaskState.COMPLETED, "Heartbeat detection task completed"
            )
            logger.info(
                f"Heartbeat detection task completed: {len(self.trials)} trials"
            )
            return True
    ...
    def _update_adaptive_parameters(self) -> None:
        ...
        if len(sync_responses) >= 5 and len(async_responses) >= 5:
            d_prime = self.d_prime_calculator.calculate_d_prime(
                [True] * len(sync_responses),
                sync_responses,
                [False] * len(async_responses),
                async_responses,
            )
    ...
    def _calculate_metrics(self) -> None:
        ...
        sync_responses = [
            trial.behavioral_response.detected
            for trial in sync_trials
            if trial.behavioral_response
        ]
        async_responses = [
            trial.behavioral_response.detected
            for trial in async_trials
            if trial.behavioral_response
        ]
        ...
        for trial in self.trials:
            if (
                trial.behavioral_response
                and trial.behavioral_response.confidence is not None
            ):
    ...
    def get_results(self) -> Dict[str, Any]:
        ...
        correct_trials = sum(
            ...
            for trial in self.trials
            if trial.behavioral_response
            and trial.is_synchronous == trial.behavioral_response.detected
        )
...
class StimulusCalibrator:
    ...
    def calibrate_interoceptive(self, stimulus_generator, n_trials: int = 30) -> float:
        """
        ...
        staircase = QuestPlusStaircase(
            parameters=QuestPlusParameters(
                stimulus_min=0.01, stimulus_max=1.0, max_trials=n_trials
            ),
    ...
    def calibrate_exteroceptive(self, stimulus_generator, n_trials: int = 30) -> float:
        """
    ...
    def run_calibration(
        self, intero_generator, extero_generator, n_trials_per_modality: int = 30
    ) -> Tuple[float, float]:
        """
        ...
        logger.info(
            f"Calibration complete: Πᵢ threshold={self.interoceptive_threshold:.4f}, "
...
class InteroceptiveDeviantGenerator:
    ...
    def __init__(self, generator_type: str = "co2", intensity: float = 0.5):
        """
...
class ExteroceptiveDeviantGenerator:
    ...
    def __init__(self, generator_type: str = "visual", intensity: float = 0.5):
        """
...
class P3bRatioCalculator:
    ...
    def calculate_ratio(
        self, interoceptive_p3b: List[float], exteroceptive_p3b: List[float]
    ) -> float:
        """
        Calculate P3b ratio.
...
class DualModalityOddballTask:
    ...
    def initialize(self) -> bool:
        ...
        try:
            self.state_machine.transition_to(
                TaskState.INITIALIZING, "Initializing oddball task"
            )
            ...
            self.intero_threshold, self.extero_threshold = (
    ...
    def run_trial(self, trial_number: int) -> Optional[OddballTrialResult]:
        ...
        try:
            ...
            # Collect response
            self.response_collector.start_collection(
                stimulus_onset_time=stimulus_onset, response_window_ms=1500.0
            )
            ...
            trial_result = OddballTrialResult(
                ...
                stimulus_parameters={},
                stimulus_modality=(
                    StimulusModality.INTEROCEPTIVE
                    if deviant_type == "interoceptive"
                    else StimulusModality.EXTEROCEPTIVE
                ),
                stimulus_intensity=(
                    self.intero_threshold
                    if deviant_type == "interoceptive"
            ...
            logger.debug(
                f"Trial {trial_number}: deviant={is_deviant}, type={deviant_type}, "
    ...
    def run(self) -> bool:
        ...
        """
        if not self.state_machine.transition_to(
            TaskState.RUNNING, "Starting oddball task"
        ):
            return False
        ...
        try:
            ...
            # Transition to completed
            self.state_machine.transition_to(
                TaskState.COMPLETED, "Oddball task completed"
            )
    ...
    def _calculate_p3b_ratio(self) -> None:
        ...
        intero_p3b = [
            ...
            for trial in self.trials
            if trial.deviant_type == "interoceptive"
            and trial.interoceptive_p3b is not None
        ]

        extero_p3b = [
            trial.exteroceptive_p3b
            for trial in self.trials
            if trial.deviant_type == "exteroceptive"
            and trial.exteroceptive_p3b is not None
        ]
    ...
    def get_results(self) -> Dict[str, Any]:
        ...
        intero_deviants = [
            trial for trial in self.trials if trial.deviant_type == "interoceptive"
        ]
        extero_deviants = [
            trial for trial in self.trials if trial.deviant_type == "exteroceptive"
        ]

        intero_detection_rate = (
            np.mean(
                [
                    trial.behavioral_response.detected
                    for trial in intero_deviants
                    if trial.behavioral_response
                ]
            )
            if intero_deviants
        ...
        extero_detection_rate = (
            np.mean(
                [
                    trial.behavioral_response.detected
                    for trial in extero_deviants
                    if trial.behavioral_response
                ]
            )
            if extero_deviants
        ...
        return {
            ...
            "p3b_statistics": (
                self.p3b_calculator.get_statistics() if self.p3b_ratio else None
            ),
        }
