...
from .multi_modal_task_manager import MultiModalTaskManager, ModalityType, TaskParadigm
from .adaptive_staircase import StaircaseParameters, StaircaseType, create_staircase
from .precision_timing import TimingController, TimingMode
...
def run_detection_threshold_experiment():
    ...
    while staircase.should_continue():
        ...
        if result:
            ...
            # Log trial result
            logger.info(
                f"Trial {trial_number}: intensity={intensity:.3f}, "
                f"detected={result.response_detected}, "
                f"RT={result.reaction_time_ms:.1f}ms"
                if result.reaction_time_ms
                else "no response"
            )
    ...
    logger.info("\n=== Experiment Complete ===")
    logger.info(
        f"Threshold estimate: {threshold_estimate.threshold:.4f} ± {threshold_estimate.std_error:.4f}"
    )
    logger.info(
        f"Converged: {threshold_estimate.converged} after {threshold_estimate.n_trials} trials"
    )
    logger.info(
        f"Confidence interval: [{threshold_estimate.confidence_interval[0]:.4f}, "
    ...
    logger.info(f"\nTiming statistics:")
    logger.info(
        f"  Mean timing error: {timing_stats['timing_statistics']['mean_abs_error_ms']:.3f}ms"
    )
    logger.info(
        f"  Timing quality: {timing_stats['timing_statistics']['timing_quality']}"
    )
...
def run_oddball_paradigm():
    ...
    sequence = timing_controller.sequencer.create_sequence(
        trial_types=trial_types, n_repetitions=1, randomize=True, random_seed=42
    )
    ...
    while timing_controller.sequencer.has_more_trials():
        ...
        modality = (
            ModalityType.VISUAL if trial_number % 2 == 0 else ModalityType.AUDITORY
        )

        # Create trial configuration
        trial_config = task_manager.create_oddball_trial(
            trial_number=trial_number, modality=modality, is_oddball=is_oddball
        )
        ...
        if result:
            logger.info(
                f"Trial {trial_number} ({trial_spec['trial_type']}): "
    ...
    logger.info(f"Mean RT: {performance['mean_reaction_time_ms']:.1f}ms")
    logger.info(
        f"Synchronization markers sent: {timing_summary['synchronization_summary']['total_markers']}"
    )
...
def run_cross_modal_threshold_comparison():
    ...
    for modality, modality_name in modalities:
        ...
        logger.info(
            f"{modality_name} threshold: {threshold.threshold:.4f} ± {threshold.std_error:.4f}"
        )
    ...
    ratio_va = normalizer.compare_thresholds(
        "visual",
        threshold_estimates["visual"].threshold,
        "auditory",
        threshold_estimates["auditory"].threshold,
    )
    if ratio_va:
    ...
    ratio_vi = normalizer.compare_thresholds(
        ...
        threshold_estimates["visual"].threshold,
        "interoceptive",
        threshold_estimates["interoceptive"].threshold,
    )
    if ratio_vi:
