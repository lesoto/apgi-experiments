...
def example_detection_task():
    ...
    if success:
        ...
        logger.info(
            f"  Threshold estimate (θ₀): {results['threshold_estimate']:.4f} ± {results['threshold_std']:.4f}"
        )
...
def example_heartbeat_detection_task():
    ...
    if success:
        ...
        if results["confidence_analysis"]:
            ...
            logger.info(
                f"  Confidence-accuracy correlation: {conf_analysis['confidence_accuracy_correlation']:.3f}"
            )
            logger.info(
                f"  Metacognitive sensitivity: {conf_analysis['metacognitive_sensitivity']:.3f}"
            )
...
def example_oddball_task():
    ...
    if success:
        ...
        logger.info(
            f"  Interoceptive detection rate: {results['interoceptive_detection_rate']:.3f}"
        )
        logger.info(
            f"  Exteroceptive detection rate: {results['exteroceptive_detection_rate']:.3f}"
        )
        logger.info(f"  Mean RT: {results['mean_reaction_time_ms']:.1f} ms")
        ...
        if results["p3b_ratio"]:
            ...
            if results["p3b_statistics"]:
                ...
                logger.info(
                    f"  Interoceptive P3b: {stats['interoceptive_p3b_mean']:.2f} μV"
                )
                logger.info(
                    f"  Exteroceptive P3b: {stats['exteroceptive_p3b_mean']:.2f} μV"
                )
...
def example_complete_session():
    ...
    logger.info("\nParameter Estimates:")
    logger.info(
        f"  θ₀ (Baseline Ignition Threshold): {detection_results['threshold_estimate']:.4f}"
    )
    logger.info(
        f"  Πᵢ (Interoceptive Precision, d'): {heartbeat_results['d_prime']:.3f}"
    )
