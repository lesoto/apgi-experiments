...
class MultiModalTaskManager:
        ...
        try:
            ...
            if self.enable_auditory:
                self.auditory_generator = ToneGenerator(
                    generator_id=f"{self.manager_id}_auditory", sample_rate=sample_rate
                )
            ...
            if self.enable_interoceptive:
                self.interoceptive_generator = CO2PuffController(
                    generator_id=f"{self.manager_id}_interoceptive", safety_enabled=True
                )
            ...
            if not self.heartbeat_synchronizer.initialize():
                logger.warning(
                    "Heartbeat synchronizer initialization failed (non-critical)"
                )
            else:
                # Connect to auditory generator for cardiac-locked tones
                if self.auditory_generator:
                    self.auditory_generator.set_heartbeat_synchronizer(
                        self.heartbeat_synchronizer
                    )
        ...
        with self.lock:
            ...
            try:
                ...
                if trial_config.modality == ModalityType.VISUAL:
                    stimulus_onset, stimulus_offset, timing_error = (
                        self._present_visual_stimulus(trial_config.visual_params)
                    )

                elif trial_config.modality == ModalityType.AUDITORY:
                    stimulus_onset, stimulus_offset, timing_error = (
                        self._present_auditory_stimulus(trial_config.auditory_params)
                    )

                elif trial_config.modality == ModalityType.INTEROCEPTIVE:
                    stimulus_onset, stimulus_offset, timing_error = (
                        self._present_interoceptive_stimulus(
                    ...
                    stimulus_onset, stimulus_offset, timing_error = (
                        self._present_multimodal_stimuli(trial_config)
                    )
                ...
                actual_duration_ms = (
                    stimulus_offset - stimulus_onset
                ).total_seconds() * 1000
                ...
                (
                    ...
                    stimulus_onset, trial_config.response_window_ms
                )
                ...
                logger.info(
                    f"Trial {trial_config.trial_number} completed: "
    ...
    def _present_visual_stimulus(
        self, params: Optional[GaborParameters]
    ...
    def _present_auditory_stimulus(
        self, params: Optional[ToneParameters]
    ) -> Tuple[Optional[datetime], Optional[datetime], float]:
    ...
    def _present_interoceptive_stimulus(
        self, params: Optional[CO2PuffParameters]
    ) -> Tuple[Optional[datetime], Optional[datetime], float]:
    ...
    def _present_multimodal_stimuli(
        ...
        if trial_config.visual_params and self.visual_generator:
            success = self.visual_generator.generate_stimulus(
        ...
        if trial_config.auditory_params and self.auditory_generator:
            success = self.auditory_generator.generate_stimulus(
                trial_config.auditory_params
            )
            if success:
                max_duration = max(
                    max_duration, trial_config.auditory_params.duration_ms
                )
        ...
        if trial_config.interoceptive_params and self.interoceptive_generator:
            success = self.interoceptive_generator.generate_stimulus(
                trial_config.interoceptive_params
            )
            if success:
                max_duration = max(
                    max_duration, trial_config.interoceptive_params.duration_ms
                )
    ...
    def _collect_response(
        self, stimulus_onset: datetime, response_window_ms: float
    ) -> Tuple[bool, Optional[datetime], Optional[float], Any, Optional[float]]:
        """
    ...
    def create_oddball_trial(
        self,
        trial_number: int,
        modality: ModalityType,
        is_oddball: bool,
        standard_intensity: float = 0.5,
        oddball_intensity: float = 0.8,
    ) -> TrialConfiguration:
        """
        ...
        if modality == ModalityType.VISUAL:
            visual_params = GaborParameters(
                ...
                spatial_frequency=2.0,
                orientation=(
                    90.0 if is_oddball else 0.0
                ),  # Different orientation for oddball
                size_degrees=2.0,
                duration_ms=200.0,
            )
            ...
            auditory_params = ToneParameters(
                frequency_hz=(
                    2000.0 if is_oddball else 1000.0
                ),  # Different frequency for oddball
                amplitude=intensity,
                duration_ms=200.0,
            )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        accuracy = (
            np.mean([r.correct for r in accuracy_trials]) if accuracy_trials else None
        )
