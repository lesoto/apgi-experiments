"""
Surprise accumulation dynamics analysis for APGI framework validation.
...
class SurpriseAccumulationAnalyzer:
    ...
    def __init__(
        self,
        apgi_equation: Optional[APGIEquation] = None,
        ...
        Args:
            apgi_equation: APGI equation calculator for surprise computation
            near_threshold_window: Window around threshold for near-threshold trials (±)
        """
        self.apgi_equation = apgi_equation or APGIEquation()
    ...
    def estimate_surprise_from_neural(
        self,
        p3b_amplitude: float,
        gamma_power: float,
        early_erp_amplitude: float,
    ...
    def estimate_threshold_from_context(
        self,
        trial_number: int,
    ...
    def analyze_trial_by_trial(
        ...
        for i in range(n_trials):
            ...
            estimated_threshold = self.estimate_threshold_from_context(
                trial_number=i, previous_ignitions=previous_ignitions
            )

            # Predict ignition probability
            predicted_probability = self.apgi_equation.calculate_ignition_probability(
        ...
        predicted_ignitions = [
            est.predicted_ignition_probability > 0.5 for est in trial_estimates
        ]
        ...
        ignition_prediction_accuracy = np.mean(
            [
                pred == actual
                for pred, actual in zip(predicted_ignitions, actual_ignitions)
            ]
        )
        ...
        try:
            ...
            predicted_probs = [
                est.predicted_ignition_probability for est in trial_estimates
            ]
            ...
            ignition_prediction_auc = self._compute_auc_manual(
                actual_ignitions, predicted_probs
            )
        ...
        if len(valid_rt_indices) > 2:
            ...
            valid_surprises = [
                trial_estimates[i].estimated_surprise for i in valid_rt_indices
            ]
            valid_threshold_distances = [
                trial_estimates[i].estimated_surprise
                - trial_estimates[i].estimated_threshold
        ...
        near_threshold_trials = [
            ...
            if abs(est.estimated_surprise - est.estimated_threshold)
            < self.near_threshold_window
        ]
        ...
        if len(near_threshold_trials) > 0:
            ...
            near_threshold_rt_variability = (
                np.std(near_threshold_rts) if len(near_threshold_rts) > 0 else 0.0
            )
        ...
        ss_residual = np.sum(
            (np.array(p3b_amplitudes) - np.array(estimated_surprises)) ** 2
        )
    ...
    def calibrate_neural_to_surprise_mapping(
        self,
        erp_components: List[ERPComponents],
        gamma_powers: List[float],
        true_surprises: List[float],
    ) -> Dict[str, float]:
        """
        ...
        def objective(weights):
            ...
            predicted_surprise = (
                w_p3b * p3b_norm + w_gamma * gamma_norm + w_early * early_norm
    ...
    def generate_report(self, result: SurpriseAccumulationResult) -> str:
        ...
        report.append(
            f"  Ignition trials: {result.n_ignition_trials} ({result.n_ignition_trials/result.n_trials*100:.1f}%)"
        )
        report.append(
            f"  Prediction accuracy: {result.ignition_prediction_accuracy:.3f}"
        )
        ...
        report.append(
            f"  RT vs Surprise: r = {result.rt_surprise_correlation:.3f}, p = {result.rt_surprise_pvalue:.4f}"
        )
        report.append(
            f"  RT vs Threshold distance: r = {result.rt_threshold_distance_correlation:.3f}, p = {result.rt_threshold_distance_pvalue:.4f}"
        )
        report.append("")

        report.append("NEAR-THRESHOLD DYNAMICS:")
        report.append(
            f"  Near-threshold trials: {result.n_near_threshold_trials} ({result.n_near_threshold_trials/result.n_trials*100:.1f}%)"
        )
        report.append(
            f"  RT variability: {result.near_threshold_rt_variability:.1f} ms"
        )
        ...
        report.append(
            f"  Threshold estimation R²: {result.threshold_estimation_r2:.3f}"
        )
        report.append(
            f"  Neural-surprise correlation: r = {result.surprise_neural_correlation:.3f}, p = {result.surprise_neural_pvalue:.4f}"
        )
        report.append("")

        report.append("VALIDATION:")
        prediction_valid = (
            result.ignition_prediction_accuracy > 0.7
            and result.ignition_prediction_auc > 0.7
        )
        rt_correlation_valid = (
            abs(result.rt_surprise_correlation) > 0.3
            and result.rt_surprise_pvalue < 0.05
        )
        neural_correlation_valid = (
            result.surprise_neural_correlation > 0.5
            and result.surprise_neural_pvalue < 0.01
        )

        report.append(f"  Prediction accuracy valid (>0.7): {prediction_valid}")
        report.append(
            f"  RT correlation valid (|r|>0.3, p<0.05): {rt_correlation_valid}"
        )
        report.append(
            f"  Neural correlation valid (r>0.5, p<0.01): {neural_correlation_valid}"
        )
        report.append("")
        report.append("=" * 70)
