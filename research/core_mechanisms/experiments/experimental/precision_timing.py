...
class PrecisionTimer:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized PrecisionTimer {timer_id} in {timing_mode.value} mode"
        )
    ...
    def mark_event_executed(self, event: TimingEvent) -> None:
        ...
        if event.timing_error_ms is not None:
            ...
            logger.debug(
                f"Event {event.event_id} executed with {event.timing_error_ms:.3f}ms error"
            )
    ...
    def get_timing_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timing_quality": (
                "excellent"
                if np.mean(abs_errors) < 0.5
                else (
                    "good"
                    if np.mean(abs_errors) < 1.0
                    else "acceptable" if np.mean(abs_errors) < 2.0 else "poor"
                )
            ),
        }
...
class TrialSequencer:
    ...
    def create_sequence(
        ...
        logger.info(
            f"Created sequence with {len(sequence)} trials (randomized={randomize})"
        )
        return sequence

    def create_blocked_sequence(
        self,
        ...
        logger.info(
            f"Created blocked sequence with {len(block_structure)} blocks, "
...
class SynchronizationManager:
    ...
    def __init__(
        self, manager_id: str = "sync_manager", enable_hardware_triggers: bool = False
    ):
        """
    ...
    def send_marker(
        ...
        logger.debug(
            f"Sent marker: {marker_type.value} (code={marker_code}) at t={timestamp:.6f}s"
        )
...
class TimingController:
    ...
    def __init__(
        ...
        self.sync_manager = SynchronizationManager(
            f"{controller_id}_sync", enable_hardware_triggers=enable_hardware_sync
        )
    ...
    def start_trial(self, trial_number: int) -> float:
        ...
        self.sync_manager.send_marker(
            SyncMarkerType.TRIAL_START, trial_number=trial_number
        )

        return trial_start

    def mark_stimulus_onset(self, trial_number: int, stimulus_id: str) -> float:
        """
    ...
    def mark_stimulus_offset(self, trial_number: int, stimulus_id: str) -> float:
        """
    ...
    def mark_response(self, trial_number: int) -> float:
        ...
        self.sync_manager.send_marker(
            SyncMarkerType.RESPONSE, trial_number=trial_number
        )
    ...
    def end_trial(self, trial_number: int) -> float:
        ...
        self.sync_manager.send_marker(
            SyncMarkerType.TRIAL_END, trial_number=trial_number
        )
