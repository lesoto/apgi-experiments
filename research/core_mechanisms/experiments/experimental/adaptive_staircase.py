...
class SimpleStaircase(AdaptiveStaircase):
    ...
    def get_threshold_estimate(self) -> ThresholdEstimate:
        ...
        if self.reversal_count < 2:
            ...
            std_error = (
                self.parameters.max_intensity - self.parameters.min_intensity
            ) / 4
        else:
            ...
            threshold = (
                np.mean(reversal_intensities)
                if reversal_intensities
                else self.current_intensity
            )
            std_error = (
                np.std(reversal_intensities) if len(reversal_intensities) > 1 else 0.1
            )
    ...
    def _was_reversal(self, trial_index: int) -> bool:
        ...
        if trial_index < len(self.intensities) - 1:
            ...
            return (
                prev_direction != 0
                and next_direction != 0
                and prev_direction != next_direction
            )
...
class PESTStaircase(AdaptiveStaircase):
    ...
    def update(self, intensity: float, response: bool) -> None:
        ...
        if was_reversal:
            self.current_step_size = max(
                self.current_step_size / 2, self.parameters.pest_min_step
            )
...
class QUESTStaircase(AdaptiveStaircase):
    ...
    def __init__(self, parameters: StaircaseParameters, staircase_id: str = ""):
        ...
        self.quest_staircase = QuestPlusStaircase(
            parameters=quest_params, participant_id="", task_id=staircase_id
        )
...
class CrossModalThresholdNormalizer:
    ...
    def normalize_threshold(self, modality: str, threshold: float) -> Optional[float]:
        ...
        logger.debug(
            f"Normalized {modality} threshold: {threshold:.4f} -> {normalized:.4f}"
        )
        return normalized

    def compare_thresholds(
        self, modality1: str, threshold1: float, modality2: str, threshold2: float
    ) -> Optional[float]:
        """
...
def create_staircase(
    ...
    """
    if (
        staircase_type == StaircaseType.SIMPLE
        or staircase_type == StaircaseType.WEIGHTED
    ):
        ...
        staircase_type == StaircaseType.QUEST
        or staircase_type == StaircaseType.QUEST_PLUS
    ):
