...
class SomaticMarkerPrimingExperiment(BaseExperiment):
    ...
    def __init__(
        self, n_participants: int = 20, screen_size: Tuple[int, int] = (1920, 1080)
    ):
        ...
        self.prime_categories = {
            "positive": [
                ...
                1710,
                1750,
            ],  # Pleasant images (babies, happy faces)
            "negative": [
                1050,
                1300,
                1930,
                2120,
                2683,
            ],  # Unpleasant images (snakes, spiders)
    ...
    def setup(self, **kwargs):
        ...
        self.total_trials = (
            len(self.prime_types)
            * len(self.coherence_levels)
    ...
    def _generate_rdk_stimulus(
        self, coherence: float, direction: str, duration: float = 1.0
    ) -> Dict:
        """Generate parameters for a random dot kinematogram."""
        n_dots = 100
        dot_positions = np.random.uniform(
            -5, 5, (n_dots, 2)
        )  # Random positions in degrees
        dot_directions = np.random.uniform(
            0, 2 * np.pi, n_dots
    ...
    def _simulate_physiological_response(self, prime_type: str) -> Dict:
        ...
        baseline_scr = np.random.uniform(
            1, 3
        )  # Skin conductance response (microsiemens)
    ...
    def run_trial(self, participant_id: int, trial_params: Dict) -> Dict:
        ...
        trial_data = {
            ...
            "trial_num": len(
                [
                    t
                    for t in self.participant_data.get(participant_id, [])
                    if t.get("prime_type") == trial_params["prime_type"]
                ]
            )
            + 1,
            **trial_params,
        ...
        for prime in self.prime_types:
            ...
            for coh in self.coherence_levels:
                ...
                if acc_key in results and rt_key in results:
                    logger.info(
                        f"  Coherence {coh:.1f}: Accuracy = {results[acc_key]:.2f}, RT = {results[rt_key]:.3f}s"
                    )
...
def run_somatic_marker_priming_experiment(n_participants=10, n_trials_per_condition=20):
    ...
    experiment = SomaticMarkerPrimingExperiment(n_participants=n_participants)
