...
class BiomarkerAnalyzer:
    ...
    def __init__(self, significance_threshold: float = 0.05):
        ...
        logger.info(
            f"BiomarkerAnalyzer initialized with threshold {significance_threshold}"
        )
    ...
    def add_patient_data(self, patient: PatientProfile) -> None:
        ...
        logger.info(
            f"Added patient {patient.patient_id} with {patient.clinical_condition.value}"
        )
    ...
    def discover_biomarkers(
        self, patients: List[PatientProfile], target_condition: ClinicalCondition
    ) -> List[ClinicalBiomarker]:
        """
        ...
        controls = [
            p
            for p in patients
            if p.clinical_condition == ClinicalCondition.HEALTHY_CONTROL
        ]

        if len(cases) < 10 or len(controls) < 10:
            raise ValidationError(
                "Insufficient data for biomarker discovery (need at least 10 per group)"
            )
    ...
    def _analyze_neural_signatures(
        ...
        target_condition: ClinicalCondition,
    ) -> List[ClinicalBiomarker]:
        ...
        for feature in p3b_features:
            case_values = [
                getattr(p, feature) for p in cases if getattr(p, feature) is not None
            ]
            control_values = [
                getattr(p, feature) for p in controls if getattr(p, feature) is not None
            ]
    ...
    def _analyze_pupillometry_features(
        self,
        cases: List[PatientProfile],
        controls: List[PatientProfile],
        target_condition: ClinicalCondition,
        ...
        pupil_features = [
            "pupil_diameter_intero",
            "pupil_diameter_extero",
            "pupil_latency_intero",
            "pupil_latency_extero",
        ]
    ...
    def _analyze_behavioral_features(
        self,
        cases: List[PatientProfile],
        controls: List[PatientProfile],
        target_condition: ClinicalCondition,
    ...
    def _create_feature_biomarker(
        ...
        effect_size = (
            (np.mean(case_values) - np.mean(control_values)) / pooled_std
            if pooled_std > 0
            else 0
        )
        ...
        reference_range = (
            np.percentile(control_values, 2.5),
            np.percentile(control_values, 97.5),
        )
    ...
    def _interpret_feature(self, feature_name: str, effect_size: float) -> str:
        ...
        feature_display = (
            feature_name.replace("_", " ").replace("p3b", "P3b").replace("hrv", "HRV")
        )

        return f"{magnitude} {direction} {feature_display} compared to healthy controls"

    def validate_biomarker(
        self, biomarker: ClinicalBiomarker, validation_patients: List[PatientProfile]
    ) -> ClinicalBiomarker:
        """
        ...
        logger.info(
            f"Validation completed: AUC={auc:.3f}, Sensitivity={sensitivity:.3f}, Specificity={specificity:.3f}"
        )

        return biomarker

    def _extract_features_for_validation(
        self, biomarker: ClinicalBiomarker, patients: List[PatientProfile]
        ...
        for patient in patients:
            ...
            if patient_features is not None:
                ...
                label = (
                    1
                    if patient.clinical_condition in biomarker.target_conditions
                    else 0
                )
                labels.append(label)
    ...
    def classify_patients(
        self, patients: List[PatientProfile], biomarker: ClinicalBiomarker
        ...
        for i, patient_id in enumerate(patient_ids):
            results[patient_id] = {
                "predicted_condition": (
                    biomarker.target_conditions[0].value
                    if predictions[i] == 1
    ...
    def generate_biomarker_report(self, biomarkers: List[ClinicalBiomarker]) -> str:
        ...
        report.append(
            f"Moderate performance (0.7 ≤ AUC < 0.8): {len(moderate_performance)}"
        )
        ...
        for biomarker in biomarkers:
            ...
            report.append(
                f"Target conditions: {', '.join([c.value for c in biomarker.target_conditions])}"
            )
            ...
            report.append(
                f"Positive Predictive Value: {biomarker.positive_predictive_value:.3f}"
            )
            report.append(
                f"Negative Predictive Value: {biomarker.negative_predictive_value:.3f}"
            )
            report.append("")
            ...
            report.append(
                f"Implementation Feasibility: {biomarker.implementation_feasibility:.3f}"
            )
            report.append("")
            ...
            if biomarker.features:
                ...
                for feature in biomarker.features:
                    ...
                    report.append(
                        f"  Effect size: {feature.effect_size:.3f} (p={feature.p_value:.3f})"
                    )
                    report.append(
                        f"  Clinical significance: {feature.clinical_significance:.3f}"
                    )
                report.append("")
