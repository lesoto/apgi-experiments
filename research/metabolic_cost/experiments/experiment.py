...
class MetabolicCostExperiment(BaseExperiment):
    ...
    def setup(self, **kwargs):
        ...
        self.n_trials_per_block = int(
            self.block_duration / (self.trial_duration + self.isi)
        )
                ...
                self.trial_sequence.append(
                    {
                        ...
                        "onset_time": block_num * self.block_duration
                        + trial_in_block * (self.trial_duration + self.isi),
                    }
                )
    ...
    def _generate_hrf(self, duration: float) -> np.ndarray:
        ...
        peak = (t / self.hrf_params["peak_delay"]) ** 6 * np.exp(
            -t / self.hrf_params["peak_disp"]
        )
        undershoot = (t / (self.hrf_params["peak_delay"] * 2)) * np.exp(
            -t / (self.hrf_params["peak_disp"] * 2)
        )
    ...
    def _simulate_neural_activity(
        self, condition: str, duration: float
    ) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            "metabolic_demand": np.abs(
                total_activity
            ),  # Absolute value to represent energy cost
        }

    def _simulate_metabolic_response(
        self, neural_activity: np.ndarray
    ) -> Dict[str, np.ndarray]:
        ...
        for t in range(1, n_samples):
            ...
            y[t] = (
                100 * E0 * (k1 * (1 - q[t]) + k2 * (1 - q[t] / v[t]) + k3 * (1 - v[t]))
            )
        ...
        return {
            ...
            "metabolic_rate": f
            * (1 - (1 - E0) ** (1 / f)),  # metabolic rate ~ flow * oxygen extraction
        }
    ...
    def run_trial(self, participant_id: int, trial_params: Dict) -> Dict:
        ...
        neural_data = self._simulate_neural_activity(
            trial_params["condition"], self.trial_duration
        )

        # Simulate metabolic response
        metabolic_data = self._simulate_metabolic_response(
            neural_data["total_activity"]
        )
        ...
        trial_data = {
            ...
            "efficiency": mean_neural_activity
            / (mean_metabolic_rate + 1e-6),  # avoid division by zero
        }
    ...
    def run_participant(self, participant_id: int):
        ...
        for i, trial_params in enumerate(self.trial_sequence, 1):
            print(
                f"  Trial {i}/{len(self.trial_sequence)} ({trial_params['condition']})",
                end="\r",
            )
    ...
    def analyze_results(self):
        ...
        for condition in self.conditions:
            ...
            results[f"{condition}_neural_activity"] = cond_data[
                "mean_neural_activity"
            ].mean()
            results[f"{condition}_metabolic_rate"] = cond_data[
                "mean_metabolic_rate"
            ].mean()
        ...
        for condition in self.conditions:
            ...
            print(
                f"  Mean Neural Activity: {results[f'{condition}_neural_activity']:.3f}"
            )
            print(
                f"  Mean Metabolic Rate: {results[f'{condition}_metabolic_rate']:.3f}"
            )
    ...
    def plot_results(self):
        ...
        conditions = ["subliminal", "conscious", "deliberation"]
        neural_means = [
            self.data[self.data["condition"] == c]["mean_neural_activity"].mean()
            for c in conditions
        ]
        metabolic_means = [
            self.data[self.data["condition"] == c]["mean_metabolic_rate"].mean()
            for c in conditions
        ]
        ...
        for condition in conditions:
            cond_data = self.data[self.data["condition"] == condition]
            plt.plot(
                cond_data["onset_time"],
                cond_data["bold_response"],
                "o-",
        ...
        plt.subplot(2, 2, 3)
        efficiency_means = [
            self.data[self.data["condition"] == c]["efficiency"].mean()
            for c in conditions
        ]
        efficiency_sems = [
            self.data[self.data["condition"] == c]["efficiency"].sem()
            for c in conditions
        ]
...
def run_metabolic_cost_experiment(n_participants=10, n_trials_per_condition=15):
    ...
    experiment = MetabolicCostExperiment(n_participants=n_participants)

    # Set up with custom parameters
    experiment.setup(n_trials_per_condition=n_trials_per_condition)
